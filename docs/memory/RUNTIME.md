...
- v0.1.56 候选延续 `ocrSchedule` 脱敏诊断：只暴露 `stopped / paused / waiting / recognizing / latched`、下一次延迟、cheap probe / full OCR 次数与最近 / 16 次滚动耗时峰值；换代、停止或旧 epoch 迟到时清零，不含截图、文本、坐标、进程标识或路径。同步保留 v0.1.55 packaged UI smoke 的 7 卡门禁，并修正跨无 Release 中间 tag 的累积发布说明。
