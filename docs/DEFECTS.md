...
| HB-098 | 英雄详情初始化被误判为来源变化并清空推荐与出装 | FIXED / UNVERIFIED（v0.1.69 候选；真实同机未验） |
...
- v0.1.69 候选记录首个来源令牌后跳过 hydration；详情页在来源快照变化时自动用新令牌重读当前英雄，不再要求用户重复点击。真实 Windows / Tencent 同机仍待验收。
