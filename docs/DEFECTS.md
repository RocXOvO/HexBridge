...
> 最后更新：2026-08-17。详细历史证据保留在 Git 历史、GitHub Actions 与 Releases；本文件只保留问题状态、当前边界和验收出口，避免 `PROJECT_MEMORY.md` 无限膨胀。
...
| HB-079 | 当前英雄等级与海克斯可选卡状态缺少受限探测 | FIXED / UNVERIFIED（v0.1.68 撤销不可行的选卡采样；仅保留等级读取） |
...
| HB-099 | 英雄榜英雄选取率徽标口径混淆、榜单卡片过大 | FIXED / UNVERIFIED（v0.1.70 已发布；真实 Windows / 真实同机验收未完成） |
| HB-100 | 实时对局仍用腾讯全局统计补位、英雄榜边框缺少炫彩卡牌效果 | FIXED / UNVERIFIED（v0.1.71 已发布；真实 Windows / Tencent 同机验收未完成） |
...
### HB-097：简洁英雄榜、独立详情与队友历史恢复

- v0.1.68 已发布：英雄榜和选人面板收敛为昵称 / 常用别名主标题、真实英雄名辅助标题，不再在榜单行重复展示胜率和选取率；英雄详情只保留来源推荐序、海克斯选择和独立 `data.dtodo` 出装模块。

### HB-098：英雄详情初始化误清空

- Renderer 初次从桥接占位状态切换到真实 RuntimeState 时，旧 watcher 会把正常 hydration 当成来源变化，并把英雄推荐与出装消息改成“请重新选择英雄”。
- v0.1.69 已发布：记录首个来源令牌后跳过 hydration；详情页在来源快照变化时自动用新令牌重读当前英雄，不再要求用户重复点击。真实 Windows / Tencent 同机仍待验收。
...
- v0.1.70 移除当前英雄与备战席中的英雄选取率徽标，避免把英雄总体指标误读为海克斯全局选取率；推荐 provider 的内部字段、来源隔离和排序规则不变，腾讯海克斯卡片的全局指标仍保留明确口径。
- 英雄榜、英雄详情海克斯图标和 Tier 卡片缩小；榜单从大面积光晕改为轻量 Tier 边框高亮，键盘选择、OP/T1–T5 分组和点击详情保持不变。
- v0.1.70 已完成全量、typecheck、lint、source bridge / UI、Windows workflow 和发布门禁；真实 Windows 多分辨率、长中文和 Tencent 同机视觉仍未验，状态保持 `UNVERIFIED`。

### HB-100：实时卡片英雄特化与英雄榜炫彩边框

- v0.1.71 已发布，将实时 OCR 三卡限制为当前英雄的 `lowest_rank_runes` 专属推荐顺序；没有专属依据的卡片不再使用腾讯全局排名或全局选取率/胜率补位，游戏内也不显示这些全局指标。英雄详情浏览页仍可按既有来源契约展示腾讯全局统计，两条场景保持隔离。
- 英雄榜继续使用紧凑卡片和小图标，选中/悬停边框改为低强度炫彩卡牌渐变，省电与 reduced-motion 模式仍关闭持续动画。
- v0.1.71 的 Windows workflow 通过 `51` files / `650` tests、4K OCR `276ms`、packaged UI / bridge、差分、双通道、公网 packaged 与五版滚动门禁；真实 Windows 多分辨率、真实 Tencent 数据和对局视觉验收仍未完成，状态保持 `UNVERIFIED`。
