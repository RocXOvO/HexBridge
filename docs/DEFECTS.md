...
| HB-097 | 英雄榜过度展示统计、详情不够简洁、队友历史易因身份接口短暂失败而缺失 | FIXED / UNVERIFIED（v0.1.68 候选；真实同机未验） |
...
- v0.1.68 候选将英雄榜和选人面板收敛为昵称 / 常用别名主标题、真实英雄名辅助标题，不再在榜单行重复展示胜率和选取率；英雄详情只保留来源推荐序、海克斯选择和独立 `data.dtodo` 出装模块。
