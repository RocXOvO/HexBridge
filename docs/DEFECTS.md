...
| HB-100 | 实时对局仍用腾讯全局统计补位、英雄榜边框缺少炫彩卡牌效果 | IN PROGRESS / UNVERIFIED（v0.1.71 候选；未完成 Windows / 真实同机验收） |
...
- v0.1.71 候选将实时 OCR 三卡限制为当前英雄的 `lowest_rank_runes` 专属推荐顺序；没有专属依据的卡片不再使用腾讯全局排名或全局选取率/胜率补位，游戏内也不显示这些全局指标。英雄详情浏览页仍可按既有来源契约展示腾讯全局统计，两条场景保持隔离。
- 英雄榜继续使用紧凑卡片和小图标，选中/悬停边框改为低强度炫彩卡牌渐变，省电与 reduced-motion 模式仍关闭持续动画。
- 本地与候选门禁完成后仍需 Windows 多分辨率、真实腾讯数据和对局视觉验收，不能升级为 `VERIFIED`。
