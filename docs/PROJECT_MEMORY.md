...
- 当前公开正式版：[v0.1.55](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.55)，Release ID `371179383`，publishedAt `2026-08-15T23:07:29Z`；tagged commit `c698060`。本版包含脱敏 OCR 调度诊断与 packaged UI smoke 7 卡门禁修正；稳定更新通道在幂等重跑后已验证，真实游戏刷新耗时仍待用户复测。
...
- v0.1.56 为当前本地候选，保留 v0.1.54 的脱敏 OCR 调度诊断并包含 v0.1.55 的打包 UI smoke 门禁修正；本候选还修正 Release notes 对“只有 tag、没有公开 Release”的中间版本累计与 compare 基线。公开正式版为 v0.1.55。v0.1.54 tag 已推送但 Windows UI smoke 因旧的 6 卡断言 fail closed，未创建 Release；v0.1.56 尚未 tag / Windows workflow / Release。真实机器性能仍待用户复测。
...
- GitHub 当前只保留 v0.1.51～v0.1.55 五个正式 Releases（v0.1.54 仅 tag）；v0.1.0～v0.1.55 tags 全部保留。本地 `release/` 为空；旧 Release / assets 按滚动策略删除但 tag / source 保留（v0.1.50 及更旧 Release 不再保留）。
