...
> 最后更新：2026-08-14
> 当前基线：公开最新正式 Release 仍为 [v0.1.19](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.19)，Release ID `370366810`，publishedAt `2026-08-14T05:13:29Z`，为 Latest、non-draft、non-prerelease。当前 v0.1.20 Windows 候选 HEAD / origin main 为 `97cc10665b70883e643ab0ab5c8b32fc02df1a18`，产品提交为 `5bcad430d738471a72cfe4990c5474ffc4d7b402`。首次 workflow_dispatch run `31775889265` / job `94691136343` 仅因 packaged UI 旧烟测错误要求 Windows idle 状态显示更新按钮而失败；产品契约实际是只有 Main 确认存在新版时才显示。只修改 `scripts/electron-ui-smoke.mjs` 的修复提交 `97cc106` 已 push，重跑 `31776277121` / job `94692248258` 基于该 HEAD 约 5m2s success，Windows 34 files / 293 passed + 1 skipped 与完整候选门禁通过。v0.1.20 尚未 tag / Release；HB-044～046 保持 `FIXED / UNVERIFIED`，HB-047 保持 `ACCEPTED RISK`。真实 WeGame、DPI、多屏、点击穿透和真实游戏性能仍未实机验证，不得写 `VERIFIED`。
...
- 状态：`FIXED / UNVERIFIED`（Windows-only 跟随链与解析测试已落地，尚无真实 WeGame / League 客户端实测）
...
- 当前证据：观察器输出采用仅含 `gameForeground / clientVisible` 布尔值的严格解析测试，退避边界、脚本静态能力与生命周期守卫进入本地测试；最终独立审查无已知 P0 / P1，v0.1.20 本地全链为 34 files / 293 passed + 1 skipped，typecheck、lint、source UI / bridge / preload / retention 与 diff-check 通过。非 Windows 平台 fail closed；source 测试不能证明真实 WeGame 的窗口枚举、DWM / PMv2 坐标、DPI / 多屏位置或 350ms 跟随成本。
- 剩余验收：Windows packaged 中覆盖单 / 多个 `LeagueClientUx`、连续移动 / 缩放、最小化 / 恢复、跨屏、负坐标、100%～150% DPI、关闭与下一次选人，确认不抢焦点、无重复 PowerShell、无窗口重建风暴、退出无残留且 CPU 有界。报告用户实机通过前不得标 `VERIFIED`。
...
- 官方政策审计结论：[Riot Games General Policies](https://developer.riotgames.com/policies/general) 与 [League of Legends developer policies](https://developer.riotgames.com/docs/lol) 要求产品不能通过未公开信息或额外实时分析形成不公平竞争优势，也不能绕过有意隐藏的玩家身份或用产品评分替代玩家自身技能判断。把当前对手的近期胜率、KDA、连胜 / 连败聚合成带价值判断的三档标签与这些边界冲突风险过高；“个人实验”不构成豁免。
- 数据源审计结论：当前国服没有经项目验证、覆盖目标模式且可可靠使用的官方对手历史数据源；GitHub 项目、未文档化 LCU 端点或客户端抓包只能作为研究线索，不能冒充 Riot 官方支持。对手身份在目标阶段不可见时不得通过注入、内存读取、屏幕 OCR、权限绕过或不明私有接口补齐，因此本轮不新增 endpoint、allowlist、IPC、缓存或 UI。
...
- tag-only Release / channel 步骤按预期 skip；公开 Latest 仍为 v0.1.19，v0.1.20 尚未 tag / Release。Windows runner 只形成候选门禁证据，不替代真实 WeGame、DPI / 多屏、点击穿透、游戏 CPU / GPU / FPS / frametime或用户同机验收；HB-044～046 保持 `FIXED / UNVERIFIED`，HB-047 保持 `ACCEPTED RISK`。
...
- HB-044～HB-046 当前均为 `FIXED / UNVERIFIED`：稳定滚动布局 / 4px 无 scale 淡入、DWM / PMv2 单一持久 Windows League 窗口观察器与 96px route 隔离小型三卡推荐窗 / 单槽最大指纹连续两帧刷新已进入 v0.1.20 Windows 候选。产品提交 `5bcad430` 后首个 run 只因旧 smoke 错误要求 idle 显示更新按钮失败；smoke-only commit `97cc106` 已修复并 push，重跑 `31776277121` / job `94692248258` 约 5m2s success。最终独审无已知 P0 / P1，Windows 34 files / 293 passed + 1 skipped 与完整候选门禁通过。v0.1.20 尚未 tag / Release，也无真实 WeGame / DPI / 多屏 / 点击穿透 / 游戏性能或用户同机证据。HB-046 不得恢复 HB-029 已移除的全屏 / 顶部黑屏窗口。
