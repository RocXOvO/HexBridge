...
> 最后更新：2026-08-13
> 当前基线：公开最新正式 Release 为 `v0.1.11`，annotated tag 固定指向产品提交 `20debe3483d8747008de240a9c9a1adcb2304c08`，tag object 为 `4141f3ed…`；`v0.1.12` 候选尚未 tag / Release。此前误把“删除旧版”理解为删除 GitHub 历史 Release，导致 `v0.1.0`～`v0.1.10` 中已发布版本的远端 Release / assets 被删除且不可恢复；其 Git tags 与源码历史仍全部保留；从 v0.1.11 起必须保留以后全部正式 GitHub Releases / assets / tags，只清理本地 `release/`。`v0.1.12` 候选包含 HB-020 / HB-022 交接保留、HB-024 整卡校准到标题 ROI / 保存前 3/3 OCR、HB-025 原子与可配置热键、HB-026 默认关闭自动 OCR + 2 秒低分辨率门控、HB-027 普通未启动提示，以及独立更新页、英雄榜 / UI / icon、远端 Release 保留与本地 cleanup 门禁。候选在 Windows workflow_dispatch run `31681231963` / job `94386905747` 中仅失败于 packaged UI calibration；此前 build、真实 4K ONNX、179 passed + 1 Windows skip、lint、typecheck、pack 与 metadata 均通过。根因是新 title-band 模板以 `v-show` 调用 `looksLikeWholeCard(rects[slot])`，初始 rect 未定义时 Vue 首次 render / mount 抛错，出现 route ready / bridge true 但 `appChildren=0`。dirty 修正草案把 helper 改为 optional guard并用 `v-if` 避免无 rect 求值，新增 undefined / null 测试；目标 6 tests、typecheck、lint、diff-check和独立 source UI 已重跑通过，但该修正尚未复审、commit / push 或重试 Windows。HB-020 / HB-022 及 HB-023～HB-027 一律保持 `IN PROGRESS`，不得写 `FIXED` / `VERIFIED`。项目无商业代码签名，公开版本边界仍以 v0.1.11 为准。
...
- 英雄榜信息减法：所有职业 / 定位名称必须使用简体中文；移除角色列和角色筛选中重复、用户价值低的展示，不在同一卡片 / 行内重复表达同一角色信息。Tier 不再作为额外拥挤列或浮动徽标，改为英雄卡背景条 / 边缘色带，同时保留文本或无障碍语义，不能只靠颜色区分。
...
- 验收边界：上述均是用户确认的后续界面目标，不代表当前 v0.1.11 已实现。验收需覆盖 1080p / 2K / 4K、100%～150% DPI、长中文英雄 / 职业名、三档视觉模式、reduced-motion、窗口可见性和 InProgress；通过逐页视觉快照、Windows packaged 人工可读性与后台 CPU / GPU / 重绘测量后才可写已完成。
...
- OCR / 浮窗验收：真实国服 WeGame 无边框对局中，进入 launching / active 上下文后应继续显示本局英雄 / 数据版本；默认关闭自动 OCR 时由按钮 / 当前配置快捷键做单次识别，用户显式开启后才按 2 秒低分辨率门控周期自动识别，并基于本局英雄详情生成推荐浮窗。选人→游戏过程中不得因 LCU transport 交接而出现上下文闪空、扫描停表或浮窗永久缺失；只有本局上下文明确定义为结束 / 换代时才停用并清除，同局 LCU 重连不得破坏当前 generation。
...
- 必须验证的验收标准：用可控 phase / snapshot 序列覆盖 `ChampSelect(英雄A, queue 2400/3270) → GameStart(空 session) → InProgress(空 session) → Reconnect → EndOfGame`，断言 A 的英雄 / 详情 / 推荐上下文在比赛阶段保留并只在结束时清理；覆盖第二局英雄 B、其他队列和过期详情返回，断言不串局。Windows packaged 实机还需验证对局信息、自动 / 当前配置快捷键 OCR 和推荐浮窗全链路。
...
- 通道 / 发布安全实现：通道发布 / 读取遵守单调版本。发布前 preflight 拒绝低于 public channel 的候选，并在任何 Release / channel 写入前分页枚举公开 stable Releases，发现更高版本立即 fail closed；远端 Release 若已是同版本，则要求五项资产及 metadata 全部一致后才 no-op；候选版本的 Release 已存在但不满足同版本一致性时拒绝覆盖。softprops 配置 `overwrite_files:false`；GitHub Actions concurrency 使用 `queue` 且 `max` 有界。`update-channel` 当前公开指向 `v0.1.11`。旧 workflow 曾在 public packaged 检查后删除严格低版 stable Release / assets；这是对用户“删除旧版”含义的错误实现，下一版发布前必须移除远端 cleanup。以后所有正式 GitHub Releases / assets / tags 永久保留，任何 workflow 都不得删除远端历史 Release。
- 当前验证：`v0.1.11` 已完成 Windows 预发布与 tag workflows，public channel / packaged public check 和五项 Release assets 均完成；由于此前错误 cleanup，目前远端确实只剩该 Latest Release，已删除的旧 Release / assets 不可恢复且不得伪称存在。从 v0.1.11 起保留当前及后续全部正式 Release / assets / tags。installed 旧版本对真实 GitHub 的 check / download / `quitAndInstall` / UAC / 替换未完成，因此 HB-021 整体仍不得写为 `VERIFIED`。旧版本用户此次建议从当前 Release 页手动覆盖安装 v0.1.11。
...
- 验收标准：Windows workflow / packaged 验证通过后，仍必须在报告问题的同一台 Windows + 国服 WeGame 上真实进入已确认的 3270 自定义 ChampSelect，并另行确认正式匹配实际 queue ID；2400 路径也必须保留回归。authority 应正确绑定同一进程的多来源 / 轮换并隔离进程复用，仲裁应脱离长期空 candidate，外部 authority 与 known terminal 残留不得误切换或覆盖本局。当前英雄一经可靠确认，主界面与紧凑选人浮窗应显示同一英雄；actions fallback、event-only trailing poll、最后换英雄、进入 GAME_STARTING / 游戏进程接管、active、终局与第二局都要保持正确 generation、清理和换代。只有报告用户同机状态链证明问题关闭后才可标为 `FIXED / UNVERIFIED`；再覆盖完整一局和第二局后才可考虑 `VERIFIED`。仅凭局部 tests、CI、production 回放或其他机器成功不得升级状态。
...
- 当前 dirty 发布草案：release workflow 中旧的 `gh release delete` 远端 cleanup 已移除，CI 增加静态 retention verifier，拒绝 workflow 出现远端 Release / tag 删除命令。`pack:win` 在构建前运行本地 `release/` cleanup：目标被固定解析为仓库根下精确 `release` 目录，拒绝该目录为符号链接或目标逃逸，随后清空其中既有条目，使本次构建结束后只留下当前产物；测试断言目录外文件不受影响。当前实现采用“精确目录预清空”而不是按 semver 逐文件保留窗口；最终只读审查与 macOS 交叉 pack 已通过，但 Windows workflow 尚未完成，且仍没有旧 / 新 blockmap 网络回读或差分下载证明。
...
- 状态：`IN PROGRESS`；Windows packaged UI 已暴露校准首帧崩溃，guard 修正仅为未复审 dirty 草案；Windows retry 与真实 4K 现场链尚未完成，不得写 `FIXED` / `VERIFIED`。
...
- 确定根因与修正草案：title-band 新模板使用 `v-show`，即使元素不可见仍会先求值 `looksLikeWholeCard(rects[slot])`；初始 `rects[slot]` 为 `undefined`，旧 helper 读取其字段并在 Vue 首次 render 抛错。当前草案让 `looksLikeWholeCard` 接受 optional rect 并在缺失时返回 false，同时把 title-band 改为 `v-if`；新增 undefined / null 回归测试。目标 6 tests、typecheck、lint、diff-check与独立 source UI standalone 已通过，但尚未最终复审、提交、推送或重跑 Windows，因此不能把 Windows 首帧范围标为已修复。
...
2026-08-13 `v0.1.12` dirty 候选基线（首次 Windows run 失败；修正草案未推送 / 未重试）：
...
- 首帧 guard 修正后只运行目标 6 tests、typecheck、lint、`git diff --check` 与独立 source UI standalone，均通过。该增量尚未完成复审、最新完整全量、commit / push 或 Windows retry，不能用失败前的 179+1 与 macOS 交叉包证明修正后的 packaged 状态。
...
- 上述结果已运行但未通过 Windows workflow；packaged UI calibration 首帧失败，修正后尚未 retry。实时 4K / DPI / 多屏校准、真实 WeGame 交接 / LCU 恢复、Windows 全局快捷键和游戏性能采样也未完成；title-only ONNX fixture 与 macOS 交叉打包不等于现场 OCR / Windows 应用全链路。HB-020 / HB-022 / HB-023～HB-027 以及 UI 长期目标状态均不得升级为 `FIXED` / `VERIFIED`。
...
- HB-021：当前 `IN PROGRESS`。`v0.1.11` 延续 raw stable channel、GitHub fallback、官方资产 allowlist、错误分类、provider 结果绑定、单调 / 并发保护，public channel 和 packaged public smoke 已通过；仍缺 installed 旧版本真实 check / download / `quitAndInstall` / UAC / 实际替换链。旧版本用户此次建议从当前 Release 页手动覆盖安装 v0.1.11。
...
- HB-023：当前 `IN PROGRESS`。dirty workflow 已删除远端 cleanup 并新增 retention 静态门禁，本地 pack 前 cleanup 受精确目录 / symlink / 越界守卫，最终审查与 macOS 交叉 pack 已通过；仍需 Windows workflow，以及真实 installed N→N+1 的旧 / 新 blockmap、`Range` / `206`、传输量、fallback 与安装验证。
- HB-024：当前 `IN PROGRESS`。Windows packaged UI 已确认 title-band 对 undefined rect 求值会让校准页首次 mount 崩溃；optional guard + `v-if` 草案及 undefined / null 目标测试通过，但尚未复审 / 推送 / Windows retry。整卡→标题 ROI、保存前 3/3、sender 隔离与真实 title-only ONNX 证据继续保留；仍缺连续实时 OCR 文本 / 置信度、Windows packaged 全链、现场 4K / DPI / 多屏、手动按钮 / 热键一致性验证。
...
- 当前 Git / 版本：公开最新 Release 为 `v0.1.11`，annotated tag 指向产品提交 `20debe3483d8747008de240a9c9a1adcb2304c08`，tag object 为 `4141f3ed…`，public channel 为 `0.1.11`。`0.1.12` 首次 workflow_dispatch 已失败；当前 title-band guard 修正尚未复审、commit / push、retry、tag 或 Release，不得把失败 run 或 macOS 交叉产物当正式资产。由于此前错误的远端 cleanup，当前事实上只有 v0.1.11 Release / assets 仍存在，`v0.1.0`～`v0.1.10` 中已发布版本的 Release / assets 已删除且不可恢复；远端 `v0.1.0` 至 `v0.1.11` Git tags 和源码历史全部原位保留。以 v0.1.11 为新保留起点，下一版起必须累积保留全部正式 GitHub Releases / assets / tags；后续记忆提交可推进 main，不得移动或改写产品 tag。
...
- 远端保留与差分发布契约（HB-023，dirty 候选）：workflow 中旧的远端 Release cleanup 已删除，并在 test / pack 前运行静态 verifier 拒绝已知 Release / tag 删除命令；v0.1.11 及以后全部正式 Releases、EXE / ZIP / blockmap / metadata / checksum assets 和 tags 仍必须永久保留。所有受支持源版本的 versioned blockmap 应随其 Release 保留，必要时再同步到独立、不可变的官方稳定通道；新版本发布还须回读确认旧 / 新 blockmap 后才原子更新 `latest.yml`。静态 verifier 与 Windows pack / metadata 已在失败 run 中通过，但 workflow 总体未通过，且 verifier 不是完整语义审计。
- 本地构建产物清理契约与候选：用户所说“发布新版删除旧的”仅指开发 / CI 工作区的本地 `release/` 构建产物。当前 `pack:win` 在构建前把目标固定为仓库根下精确 `release/`，拒绝 release 目录 symlink 与解析后目标逃逸，再清空其中全部旧条目；因此本次构建后自然只剩当前版本，而不是按 semver 逐项清理。单测确认目录外文件保留和 symlink 拒绝。远端 GitHub Releases / assets / tags 永远不属于本地清理目标；最终审查与 macOS 交叉 pack 已通过，仍待 Windows 门禁。
...
- `v0.1.0` 曾含 HB-012 preload 阻断缺陷，其 Release 已删除，不能作为功能可用基线。当前公开推荐版本为 `v0.1.11`；但 tagged packaged smoke 通过仍不代表真实 WeGame / LCU / 对局 OCR 实机验收。
- `v0.1.11` 是当前公开推荐版本，且因历史错误 cleanup 暂为唯一仍存在的正式 Release；从该版本起的远端历史资产必须保留。HB-020 与 HB-022 仍处于 `IN PROGRESS`，用户已真实复现交接空窗丢英雄，不能把该版本写成已完成同机交接验证。HB-014 仅在首帧黑屏 / packaged Windows 受控截图与 Esc 恢复窄范围 `VERIFIED`；HB-013、HB-015～HB-018 及 HB-014 的实机剩余范围仍为 `FIXED / UNVERIFIED`。HB-019 仅 local-feed check / download / SHA-512 / cache 窄范围 `VERIFIED`，真实 installed 客户端更新 / 安装链仍未验证。
- HB-021 当前为 `IN PROGRESS`：`v0.1.11` 已正式发布，public channel 与 packaged public check 已指向 `0.1.11`；但旧 installed 客户端的真实 GitHub check / download / `quitAndInstall` / UAC / 版本替换仍未完成。不得把 workflow、synthetic updater 或服务端资产存在等同于真实客户端升级已验证；此次建议旧版本用户手动覆盖安装 v0.1.11。
...
- GitHub Actions 仍有 Node 20 deprecated annotation。当前 workflow 中第三方 actions 固定到完整 commit SHA，runner 已临时强制它们使用 Node 24，且 `v0.1.11` 正式 run `31668236682` 成功，因此不是当前发布阻断项。
...
- 2026-08-13 | HB-020 / 实机交接 runbook | IN PROGRESS（等待用户执行） | 新增 `docs/WEGAME_HANDOFF_RUNBOOK.md`，把真实同机状态链、不变量、失败判据、终局 / 第二局、脱敏字段与 FIXED / VERIFIED 升级门槛写为仓库契约；README 提供入口 | `git diff --check` 通过；内容与 Runtime 受控诊断字段及项目记忆一致 | 文档不能替代用户实际运行，目标仍未完成 | 待报告问题的同一机器手动覆盖安装当前 v0.1.11 后执行
