...
> 最后更新：2026-08-14
> 当前基线：公开最新正式 Release 为 [v0.1.20](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.20)，Release ID `370421727`，publishedAt `2026-08-14T07:38:30Z`，public、Latest、non-draft、non-prerelease。annotated tag `v0.1.20` 解引用到产品 / 标签提交 `4aecc9199f1d95fd5dd88fce1a29645180d2a368`；tag run `31780353884` / job `94704573925` 于 `07:33:05Z`～`07:38:39Z` success，约 5m34s，Windows 36 files / 306 passed + 1 skipped 及 packaged / differential / Release / channel / public check 全部门禁通过。当前 stable 元数据路径是 `update-channel/v2/latest.yml`，version `0.1.20`、path、size `199249536` 与 SHA-512 均和正式 EXE 一致；legacy 根 `latest.yml` 保留 0.1.14 是兼容冻结而非故障。v0.1.11～v0.1.20 Releases / assets 均保留。在该正式发行基线中 HB-044 / HB-046 / HB-047 与 HB-045 总体为 `FIXED / UNVERIFIED`，只有 HB-045 synthetic native-window follow 的既有窄范围为 `VERIFIED`；HB-047 的当前 dirty 修复状态见下一段，政策 / 自定义分发继续 `ACCEPTED RISK`。正式发布不等于真实 WeGame、身份载荷、端点、隐私、DPI、多屏、点击穿透或游戏性能已验证；无商业代码签名与 Node 20 action 非阻断 annotation 边界不变。
> 当前未发布候选：版本已升至 `v0.1.21`，HB-047～HB-053 仍全部 `IN PROGRESS / UNVERIFIED`，HB-047 的政策 / 自定义分发继续 `ACCEPTED RISK`。用户已澄清“对手战绩无用”是要求修复，不是移除；任何删除对手战绩功能的方向均已完整撤回。HB-047 最终候选已收口阶段化身份 / visibility、对手与历史参与者唯一关联、错误分类、瞬态历史重试和 Runtime 有界重试；最终审查无 P0 / P1。test-only observer gate 提交 `2304ee5afc8cdbe89a39f6184d918087426c6f95` 已 push main；workflow_dispatch run `31802621240` / job `94773940537` 于 `12:58:32Z`～`13:03:52Z` success，约 5m20s。clean npm ci、audit、public 0.1.20、OCR synthetic + 真实 fixture、Windows 36 files / 350 passed + 1 skipped、lint、typecheck、retention、legacy、pack / metadata、packaged UI / bridge、differential updater、checksums 与 artifact 全过；artifact ID `9220014016`，size `473454828`，digest `sha256:3af41d279d19419befadaf9aee3f36100268bbe6d4316efa786ccfca31452003`。workflow_dispatch 的 tag-only Release / channel / public 步骤按预期 skip，仍无 v0.1.21 tag / Release，公开 Latest 仍为 v0.1.20。Windows runner 不连接真实 WeGame，不能升级 HB 状态或覆盖用户实机失败事实。
...
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 最终候选已通过无 P0 / P1 的最终审查、本地全量和 Windows workflow 候选门禁；真实国服 WeGame、真实身份 / 历史端点与用户同机仍未验证）
...
- 当前候选与验证：v0.1.20 正式 tag run `31780353884` / job `94704573925` 的 36 files / 306 passed + 1 skipped 只证明旧严格 `VISIBLE` 实现及当时发布链。v0.1.21 最终候选已实现上述阶段化可见性、唯一关联、错误分类、历史瞬态重试与 Runtime 有界重试；最终审查无 P0 / P1，本地完整 36 files / 350 passed + 1 skipped 及 source / OCR / icon / retention / diff 门禁全过。test-only commit `2304ee5afc8cdbe89a39f6184d918087426c6f95` 的 Windows workflow_dispatch run `31802621240` / job `94773940537` 又通过 Windows 36 files / 350 passed + 1 skipped 与 packaged UI / bridge、differential updater 等候选全链，但 runner 不连接真实 WeGame。真实身份 / history endpoint 和用户同机均未完成，因此仍不得写 `FIXED` / `VERIFIED`。历史端点、并发 2、2 MiB、Abort、Main-only 与 PUUID 不出 Main / 不日志 / 不进状态 / 不落盘边界未改变。
...
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 候选已通过两轮无 P0 / P1 的最终审查、最新完整本地全量并获准进入 Windows；Windows、真实 WeGame 与用户同机仍未完成）
...
- 当前 v0.1.21 候选：租约统一为 `active=12h`、`committed=10m`、`weak=15s`；空 ChampSelect、`None` 或 partial observation 不得续期，只有正向 `GAME_STARTING`、`gameClient`、独立游戏进程或可靠 augment 证据才允许进入对应长租。候选同时把伴随窗手动关闭绑定到当前 generation，本局不得自动重开、下一局可恢复。该实现尚未证明能覆盖用户真实 3270 退出链，状态不得升级。
...
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 候选已通过两轮无 P0 / P1 的最终审查并获准进入 Windows；既有 synthetic 窄证据保留，但本候选的 Windows / 真实 WeGame / 用户同机未完成）
...
- 当前 v0.1.21 候选：以真实 `LeagueClientUx` PID 为锚点，使用 Win32 客户区 / 边框关系贴边，并对小幅坐标变化做抖动抑制；同时接入本局手动关闭抑制。它仍未在用户真实 WeGame、多窗口或混合 DPI 环境验证，不得把候选逻辑或既有 fake window smoke 写成修复完成。

### HB-050 游戏内刷新海克斯后 OCR 与推荐更新过慢

- 严重度：高（刷新后的实际选择窗口有限，迟到推荐失去作用）
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 候选已通过两轮无 P0 / P1 的最终审查、最新完整本地全量并获准进入 Windows；Windows、用户同机时序与性能仍未完成）
...
- 当前 v0.1.21 候选：标题指纹变化稳定确认窗口收口为 280ms；确认刷新后最多执行 4 次有界 full OCR 重试，成功 3 / 3 后提交新组合，耗尽后不得猜测。新增目标门禁已覆盖连续 3 次 `UNRELIABLE`、第 4 次 `MATCHED` 后不再发生第 5 次 full OCR。该目标测试不等于真实刷新速度或性能闭环，重试仍必须服从 single-flight、窗口 / generation 守卫和有界停止。

### HB-051 三张海克斯必须显示英雄专属真实 pickRate

- 严重度：中高（把 rank / 排行误当作选取率会错误表达数据含义）
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 候选已通过两轮无 P0 / P1 的最终审查、最新完整本地全量并获准进入 Windows；Windows、真实上游与用户同机未完成）
...
- 当前 v0.1.21 候选：96px compact 同时携带 / 展示推荐 `1 / 2 / 3`、rank / 推荐依据与英雄专属 `pickRate`；pickRate 不参与排序。只有上游 `source` 与 `region` 同时命中明确 allowlist 才允许显示百分比，否则保持 `null / 暂无数据`；不得把 rank、全局数据或未知区域值回填成选取率。

### HB-052 选择卡牌后三卡下方 96px 推荐条不自动消失

- 严重度：高（选择完成后持续覆盖游戏画面，会把已结束的三卡界面误当作仍可选择）
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 候选已通过两轮无 P0 / P1 的最终审查、最新完整本地全量并获准进入 Windows；Windows 与用户同机仍未完成）
...
- 当前 v0.1.21 候选：主窗口继续保留最后可靠 slots；96px compact 只在卡面可靠存在时显示，并在失焦、功能禁用、刷新开始、连续两次可靠 absence 或显示满 45 秒时关闭。`autoOcr=false` 的手动成功路径只启动上述有界低成本存在性监测，不转为持续 full OCR。该超时 / absence 策略仍需真实三卡选择、刷新与性能验收。

### HB-053 中途启动 HexBridge 时无法恢复本局英雄

- 严重度：高（应用在对局中途启动时若没有当前英雄，OCR 推荐和英雄专属数据无法建立可靠上下文）
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 候选已通过两轮无 P0 / P1 的最终审查、最新完整本地全量并获准进入 Windows；Windows 与用户同机仍未完成）
- 候选实现：Main 只读缓存 current-summoner PUUID，并且只在 `active` 阶段与 gameflow team 做唯一匹配以恢复当前英雄；匹配不唯一或来源不成立时不得恢复。失败退避固定为 5000ms；新增目标门禁已覆盖首次 `null` 后 4999ms 不重试、5000ms 成功，并断言 snapshot / logger 均无 PUUID。PUUID 只可存在于 Main 有界内存，不得写日志、Renderer、IPC 业务状态、缓存、配置或磁盘。
...
2026-08-14 v0.1.21 / HB-048～HB-053 Windows 候选时间线与成功基线（尚未 tag / Release）：
...
- Release [v0.1.20](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.20) ID `370421727`，publishedAt `2026-08-14T07:38:30Z`，为 public、Latest、non-draft、non-prerelease。正式 Actions artifact ID `9211571523`，size `473444531` bytes，digest `0a85291404c942550b6687eb51e8bc95def2026cc98bf2c6b23bf19c507832d7`。
...
- 当前 stable 路径为 `update-channel/v2/latest.yml`；其 version `0.1.20`、path、size `199249536` 与 SHA-512 均和正式 EXE 一致，channel branch commit 为 `60252b27…`。legacy 根 `latest.yml` 固定保留 0.1.14 是既定兼容边界，不是更新故障。v0.1.11～v0.1.20 Releases / assets 全部保留。
...
- HB-033～HB-038 当前均为 `IN PROGRESS`。v0.1.15～v0.1.20 的 Windows 差分烟测均在受控 cache 前置条件下验证 Range / blockmap 与候选 / 正式门禁；但用户 v0.1.14→v0.1.15 仍看到 / 可能下载约 200 MB，尚无其 installed 客户端网络字节证据区分 metadata 展示与 full fallback。必须由 installed 客户端对 v0.1.20 或后续正式版复验；Windows 图标、合成器隐藏截图恢复，以及报告用户同机多轮三卡 / 中文依据 / toast 也仍未验。公开 Latest 为 v0.1.20。
...
- HB-047 当前为 `IN PROGRESS / UNVERIFIED`，政策 / 自定义分发为 `ACCEPTED RISK`：用户要求修复而非移除对手战绩，删除方向已完整撤回。v0.1.21 最终候选实现 selecting / launching=champ-select、active=gameflow 的阶段 visibility，要求本方 / team 与五个对手身份唯一且 visibility 拒绝优先；fulfilled null / error envelope 分类、历史 identity↔participant 唯一 join / 单 participant legacy fallback、500 / 429 一次重试与 404 / schema 不重试均已收口。Runtime 仅对 source / team incomplete 或 transport Abort 做 3 / 5 / 10 / 15 / 15 秒有界重试，hidden / ambiguous 不重试，成功停止、耗尽手动。最终审查、本地 36 files / 350 passed + 1 skipped 与 Windows run `31802621240` 候选全链均通过，但 runner 不连接真实 WeGame，真实身份 / endpoint / 用户同机尚未完成，不得标 `FIXED` / `VERIFIED`。
- HB-048～HB-052 当前均为 `IN PROGRESS / UNVERIFIED`：v0.1.20 用户实机失败事实保持有效；v0.1.21 候选已加入统一 `active=12h / committed=10m / weak=15s` 租约且空 ChampSelect / `None` / partial 不续期、真实 Ux PID 锚定与 Win32 贴边 / 抖动抑制 / 本局手动关闭、280ms 刷新确认 + 最多 4 次 full OCR、compact rank / 依据 / allowlisted pickRate，以及可靠卡面 / 两次 absence / 45 秒小条生命周期。最终审查无 P0 / P1；当前完整本地 36 files / 350 passed + 1 skipped、typecheck / lint / source bridge / UI、OCR synthetic + 真实 4K 141ms、retention / icon / diff-check 全过，但 Windows / 真实 WeGame / 用户同机未完成，所有项不得写 `FIXED` / `VERIFIED`。
- HB-053 当前为 `IN PROGRESS / UNVERIFIED`：v0.1.21 候选使用 Main-only current-summoner PUUID 缓存，只在 active gameflow team 中唯一匹配以恢复中途启动时的本英雄；首次 `null` 后 4999ms 不重试、5000ms 才成功，snapshot / logger 与 Renderer / 磁盘均不得含 PUUID。目标 2 files / 26 tests 和上述最终完整门禁已通过，但 Windows / 真实 WeGame / 用户同机仍无证据，不得标 `FIXED` / `VERIFIED`。
...
- 当前更新通道 / Git：stable 元数据使用 `update-channel/v2/latest.yml`；version `0.1.20`、path、size `199249536` 与 SHA-512 均和正式 EXE 一致，channel branch commit 为 `60252b27…`。legacy 根 `latest.yml` 保留 0.1.14 是兼容冻结，不是发布或更新故障。main 在本次记忆更新提交后可领先 v0.1.20 tag，不预写自身未知 hash；Release 产品源码固定为 tag 解引用的 `4aecc9199f1d95fd5dd88fce1a29645180d2a368`。v0.1.11～v0.1.20 正式 Releases / assets / tags 均保留，不得移动、改写或删除。
...
- GitHub Actions 仍有 Node 20 deprecated annotation。当前 workflow 中第三方 actions 固定到完整 commit SHA，runner 已临时强制它们使用 Node 24；`v0.1.20` 正式 run `31780353884` 已成功，因此该 annotation 仍为非阻断维护项。
