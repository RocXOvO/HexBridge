...
- 当前候选为 `v0.1.59`，公开正式版为 [v0.1.58](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.58)；本地全量 `50` files / `627` passed + `1` skipped，typecheck、lint、diff-check 与渲染/增量 OCR 定向回归通过。候选移除整组三卡外层过渡，短暂隐藏/恢复保留可靠槽位签名，只让变化卡片动画；真实 WeGame 单卡刷新、帧时间仍 `UNVERIFIED`。v0.1.59 尚未 commit / push / Windows workflow / tag / Release。
