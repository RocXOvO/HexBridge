...
- 当前公开正式版为 [v0.1.59](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.59)；本地全量 `50` files / `628` passed + `1` skipped，typecheck、lint、diff-check 与渲染/增量 OCR 定向回归通过。v0.1.59 移除整组三卡外层过渡，短暂隐藏/恢复保留可靠槽位签名，只让变化卡片动画；真实 WeGame 单卡刷新、帧时间仍 `UNVERIFIED`。
- v0.1.60 当前为未发布候选：只在显式可选卡诊断采样中增加一次性 `allgamedata`，固定本机 `127.0.0.1:2999`、2 MiB / 超时 / Main-only 脱敏；不接入后台轮询、不把等级或事件字段当作选卡状态。用户已提供的三份真实摘要尚未发现稳定选卡字段。本地 `50` files / `630` passed + `1` skipped、typecheck、lint、diff-check 已通过；等待候选版再次采样，不得预写 Windows、Release 或 `VERIFIED` 结果。
...
- GitHub 当前只保留 v0.1.55、v0.1.56、v0.1.57、v0.1.58、v0.1.59 五个正式 Releases（v0.1.54 仅 tag）；v0.1.0～v0.1.59 tags 全部保留。本地 `release/` 为空；旧 Release / assets 按滚动策略删除但 tag / source 保留。
