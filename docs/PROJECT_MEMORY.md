...
- v0.1.56 已推送不可变 tag，workflow `31914401071` 正在运行，尚无 Release；它保留 v0.1.54 的脱敏 OCR 调度诊断并包含 v0.1.55 的打包 UI smoke 门禁修正，还修正 Release notes 对“只有 tag、没有公开 Release”的中间版本累计与 compare 基线。公开正式版为 v0.1.55。v0.1.54 tag 已推送但 Windows UI smoke 因旧的 6 卡断言 fail closed，未创建 Release。真实机器性能仍待用户复测。
