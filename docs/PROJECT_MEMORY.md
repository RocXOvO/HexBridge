...
- 通道 / 发布安全实现：通道发布 / 读取遵守单调版本。发布前 preflight 拒绝低于 public channel 的候选，并在任何 Release / channel 写入前分页枚举公开 stable Releases，发现更高版本立即 fail closed；远端 Release 若已是同版本，则要求五项资产及 metadata 全部一致后才 no-op；候选版本的 Release 已存在但不满足同版本一致性时拒绝覆盖。softprops 配置 `overwrite_files:false`；GitHub Actions concurrency 使用 `queue` 且 `max` 有界。`update-channel` 当前公开指向 `v0.1.7`。`v0.1.8` 候选又增加 Release 保留策略：只在当前 strict-semver stable Release 已公开且 public packaged 检查通过后删除严格低版 stable Release / assets；cleanup 再次拒绝更高 stable，draft / prerelease 不删，所有 tags / 源码历史永久保留。
- 当前验证：`v0.1.7` 已经 Windows 预发布与 tag workflows，public channel / packaged public check 和五项 Release assets 均完成；目前只保留该 Latest Release。installed 旧版本对真实 GitHub 的 check / download / `quitAndInstall` / UAC / 替换未完成，因此 HB-021 整体仍不得写为 `VERIFIED`。
...
`v0.1.7` 当前正式发布与 `v0.1.8` 本地候选：

- `v0.1.7` tag / 当前 HEAD 为 `ea0c7e2`，GitHub Release 为公开、非 draft / prerelease 的 Latest；`update-channel/latest.yml` 版本、URL、size 和 SHA-512 与正式 EXE 对齐。
- 五项正式资产：EXE 199,023,315 bytes / SHA-256 `c9873c8799f8d3d71890cb798df793b54f192cee21265b9605e2b702ba46ad58`；blockmap 201,427 bytes / `a888bc1a920da93e07c7b13b1323d42623f853da2c5e9a4563e59613233f31a3`；ZIP 274,209,255 bytes / `21ae2487b64dc7d9ccef3f1459755e68994d07ee860f2c2bb6c10203f4cd5989`；`latest.yml` 343 bytes / `f4e59e26461c50ac1f9191d117b6bdc90ffa981f347bd5f9a08630025c749f1e`；`SHA256SUMS.txt` 180 bytes / `b07671bdc0c7d22efed089177dd3f87b6309bf1aae820e09f4a5fdb3fd5e0e7b`。
- 当前本地 `v0.1.8` 候选包含 HB-020 reducer / 浮窗 / 脱敏决策记录和 Release 保留策略；本地截至本记录已通过 13 test files / 99 passed + 1 Windows-only skipped、lint、typecheck、source bridge / UI smoke、public `0.1.7` channel verify、production audit 0、真实 GitHub 只读 preflight 和 diff-check。Release policy fixture 明确覆盖数值 semver、较低 stable、较高 draft / prerelease 和较高 stable fail-closed。它尚未 commit / push / Windows Actions / tag / Release，也没有真实 WeGame 同机复验，HB-020 保持 `IN PROGRESS`。
- 最终增量后的 macOS `pack:win + checksums` exit 0，updater metadata verifier 核对 `version=0.1.8`、EXE path / size / SHA-512 与唯一非空 blockmap；交叉候选 EXE 198,535,875 bytes / SHA-256 `46c55bc9224dc1a988a903be852898d8c6076e87ccdc8b28894941562ac8d8a2`，ZIP 274,099,763 bytes / SHA-256 `5634314c3dd9f86ca4d0340c71e9c3a2e8b6ca7bed1821b0937dbb054f26f83a`。这些仅是 macOS 交叉构建证据，正式资产必须以后续 Windows tagged Actions 为准。
- 最终只读审查确认 HB-020 reducer、preflight 和 stable-only cleanup 当前无 P0 / P1，并只批准提交及 Windows workflow_dispatch；不得跳过 Windows 预发布直接打 tag，也不得把审查 / CI 外推为真实 WeGame 同机验证。
...
- HB-021：当前 `IN PROGRESS`。`v0.1.7` 已完成 raw stable channel、GitHub fallback、官方资产 allowlist、错误分类、provider 结果绑定、单调 / 并发保护、public channel 和 packaged public smoke，并已正式发布；仍缺 installed 旧版本真实 check / download / `quitAndInstall` / UAC / 实际替换链。旧 `v0.1.5` / `v0.1.6` 无法远程修复，需从当前 Release 页手动安装最新正式版一次。
...
- `v0.1.0` 曾含 HB-012 preload 阻断缺陷，其 Release 已删除，不能作为功能可用基线。当前公开推荐版本为 `v0.1.7`；但 tagged packaged smoke 通过仍不代表真实 WeGame / LCU / 对局 OCR 实机验收。
...
- HB-021 当前为 `IN PROGRESS`：`v0.1.7` 已正式发布，public channel 与 packaged public check 已指向 `0.1.7`；但旧 installed 客户端的真实 GitHub check / download / `quitAndInstall` / UAC / 版本替换仍未完成。不得把 workflow、synthetic updater 或服务端资产存在等同于真实客户端升级已验证。
