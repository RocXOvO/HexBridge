...
> 当前未发布候选：版本已升至 `v0.1.21`，HB-047～HB-053 仍全部 `IN PROGRESS / UNVERIFIED`，HB-047 的政策 / 自定义分发继续 `ACCEPTED RISK`。用户已澄清“对手战绩无用”是要求修复，不是移除；任何删除对手战绩功能的方向均已完整撤回。HB-047 最终 dirty 候选已收口阶段化身份 / visibility、对手与历史参与者唯一关联、错误分类、瞬态历史重试和 Runtime 有界重试；最终审查无 P0 / P1。当前完整本地证据为 36 files / 350 passed + 1 skipped、typecheck、lint、source bridge / UI、OCR synthetic、真实 4K fixture（141ms）、icon、retention 与 diff-check 全过。candidate commit `c12b3a4` 已 push main；workflow_dispatch run `31799661557` / job `94764404770` 在 Windows `npm test` 因 lifecycle fake BrowserWindow 缺少 `getNativeWindowHandle` 而失败，失败前 prechecks、audit、public 与 OCR 已过，未进入 build / Release。当前 dirty nativeHandle 修复继续以方法 / Buffer 守卫与 catch 后 fail closed，且审查无 P0 / P1；P2 全零 Buffer 仅为非阻断边界，仍待 commit / push 与 Windows retry。公开 Latest 仍为 v0.1.20，尚无 v0.1.21 tag / Release；不得把本地全量或审查外推成 Windows、真实 WeGame 或用户同机已验证，也不得覆盖 v0.1.20 用户实机失败事实。
...
- 本地 / Windows 证据：当前最终完整本地门禁为 36 files / 350 passed + 1 skipped、typecheck、lint、source bridge / UI、OCR synthetic、真实 4K fixture（141ms）、retention、icon 与 `git diff --check` 全过，最终审查无 P0 / P1。candidate commit `c12b3a4` 已 push main；workflow_dispatch run `31799661557` / job `94764404770` 在 Windows `npm test` 因 lifecycle fake BrowserWindow 缺少 `getNativeWindowHandle` 而失败，失败前 prechecks、audit、public 与 OCR 已过，且未进入 build / Release。当前 dirty nativeHandle 修复继续以方法 / Buffer 守卫与 catch 后 fail closed，修复审查无 P0 / P1，P2 全零 Buffer 为非阻断边界，仍待 commit / push 与 retry。真实 WeGame 与用户同机均未完成；HB-048～HB-053 全部继续 `IN PROGRESS / UNVERIFIED`。
