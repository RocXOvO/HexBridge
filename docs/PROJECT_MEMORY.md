...
- v0.1.55 已创建公开 Release（Release ID `371179383`，publishedAt `2026-08-15T23:07:29Z`，五资产完整）；首次 workflow 在稳定通道发布步骤 fail closed，当前正按幂等流程重跑该步骤。v0.1.56 为当前本地候选，保留 v0.1.54 的脱敏 OCR 调度诊断并包含 v0.1.55 的打包 UI smoke 门禁修正；本候选还修正 Release notes 对“只有 tag、没有公开 Release”的中间版本累计与 compare 基线。公开正式版为 v0.1.55。v0.1.54 tag 已推送但 Windows UI smoke 因旧的 6 卡断言 fail closed，未创建 Release；v0.1.56 尚未 commit / push / Windows workflow / Release。真实机器性能仍待用户复测。
