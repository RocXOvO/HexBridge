...
- 当前候选版为 `v0.1.66`，公开正式版仍为 [v0.1.64](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.64)；v0.1.65 已推送 tag，但其打包 UI smoke 使用旧入口选择器而 fail closed、未创建公开 Release。v0.1.66 修正该发布门禁并保留来源卡片配置入口；HB-093/HB-094/HB-095 的真实 Tencent 与用户同机验收不由 CI 外推，尚未 commit / push / Windows / tag / Release。
...
17. HB-095：用 v0.1.66 Windows 客户端确认英雄详情推荐在 data.dtodo Key 缺失 / 失败时仍可显示，且从来源卡进入 Key 页面；真实 Tencent 与同机视觉仍不由自动化外推。
