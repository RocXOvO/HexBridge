...
> 最后更新：2026-08-14
> 当前基线：公开最新正式 Release 仍为 [v0.1.19](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.19)，Release ID `370366810`，publishedAt `2026-08-14T05:13:29Z`，为 Latest、non-draft、non-prerelease。当前 v0.1.20 产品 / Windows 候选 HEAD 与 origin main 为 `cd8992ae6d8c34bd4b5d8ce68e8885f41607ac0e`，已 push。workflow_dispatch run `31779716617` / job `94702618479` 于 `2026-08-14T07:23:09Z`～`07:28:33Z` success，约 5m24s；clean npm ci / Electron hydrate / audit high 0 / public v0.1.19 channel、OCR synthetic + real fixture、Windows 36 files / 306 passed + 1 skipped、lint、typecheck、retention、legacy、pack / updater metadata、packaged UI / bridge（含 native window follow synthetic）、differential update、checksums 与 artifact 全过，tag-only 步骤按预期 skip。artifact ID `9211328981`，size `473444447` bytes，digest `93d17c3eeaf5aaae6ca27f9b52b626de38574a71121d68e6e1317671886aa01c`。v0.1.20 仍未 tag / Release；HB-044 / HB-046 保持 `FIXED / UNVERIFIED`，HB-045 只有 Windows synthetic native-window follow 窄范围 `VERIFIED`、总体仍为 `FIXED / UNVERIFIED`；HB-047 实现保持 `FIXED / UNVERIFIED`，政策与自定义分发边界保持 `ACCEPTED RISK`。本次 CI 不连接真实 WeGame，不能把真实 2400 / 自定义 3270 身份载荷、对手端点、隐私、混合 DPI、多屏、最小化、点击穿透或游戏性能标为 `VERIFIED`。
...
### 3.2.1 可整合数据源审计（多数方案未实现；HB-047 窄实验已进入 Windows 候选）
...
4. **本地 LCU Match History（默认关闭的 HB-047 个人实验，Windows 候选已接入）**
...
- 状态：`FIXED / UNVERIFIED`（v0.1.20 候选 `cd8992ae6d8c34bd4b5d8ce68e8885f41607ac0e` 已 push 并通过 Windows workflow_dispatch；尚未 tag / Release，也未在真实国服 WeGame 验证端点和身份载荷）
...
- 当前验证：最后一次阶段化身份来源修正后，本地完整 `npm test` 为 36 files / 306 passed + 1 skipped；typecheck、lint、source bridge、source UI 与 diff-check 全过，独立终审无已知 P0 / P1。候选 `cd8992a` 的 Windows run `31779716617` / job `94702618479` 又通过 clean dependencies、OCR、Windows 36 files / 306 passed + 1 skipped、lint / typecheck、packaged UI / bridge、差分更新和完整候选门禁。测试覆盖严格可见身份、恰好 5 人、固定 allowlist、未满足门禁不查询、Abort、少于 8 局不分档、配置默认关闭 / 迁移和 Runtime 生命周期；这些自动化不连接真实国服 WeGame，不能把端点、真实对手载荷或隐私边界标为 `VERIFIED`。
...
- HB-044～HB-046 当前均为 `FIXED / UNVERIFIED`：v0.1.20 Windows 候选 HEAD `cd8992ae` 已 push；run `31779716617` / job `94702618479` 约 5m24s success，Windows 36 files / 306 passed + 1 skipped 与完整候选门禁全过。packaged bridge 继续覆盖 smoke-only 原生 WinForms `Fake LeagueClientUx` 移动窗口和生产 observer 跟随；因此 HB-045 的 Windows synthetic native-window follow 窄范围可记 `VERIFIED`，但真实 WeGame、多屏 / 混合 DPI / 最小化仍未验，HB-045 总体不升级。v0.1.20 尚未 tag / Release，HB-044 / HB-046 也无真实点击穿透 / 游戏性能或用户同机证据；HB-046 不得恢复 HB-029 已移除的全屏 / 顶部黑屏窗口。
- HB-047 当前实现为 `FIXED / UNVERIFIED`，政策 / 自定义分发为 `ACCEPTED RISK`：v0.1.20 候选 `cd8992ae` 已 push 并通过 Windows workflow_dispatch；默认关闭、仅本机、受支持海克斯队列 `{2400, 3270}`（包含用户实际使用的自定义 3270）的阶段化身份 / 有界历史查询、Main-only 隐私和少于 8 局不评分契约不变。Windows 36 files / 306 passed + 1 skipped、packaged UI / bridge 与候选全链通过，但 CI 不连接真实 WeGame，也未验证真实 2400 / 自定义 3270 身份载荷、history endpoint、隐私或性能；v0.1.20 未 tag / Release，不得标 `VERIFIED` 或声称公开分发获 Riot 批准。
...
- v0.1.19 发布状态：annotated tag object `a44b758fcbe05ec3744b4c6afa57b1622929c9ef` 指向产品 / 候选记忆 commit `0b30064d399c69d7edc8f082d0ef27b40bc8c5d3`；正式 run `31772021173` / job `94679735360` success，约 5m57s。Release [v0.1.19](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.19) ID `370366810`，publishedAt `2026-08-14T05:13:29Z`，为 Latest、non-draft、non-prerelease；public packaged 返回 channel 0.1.19 / no update。
- v0.1.19 正式资产：EXE / blockmap / ZIP / `latest.yml` / `SHA256SUMS.txt` 五项大小和 SHA-256 见第七节；public channel 独立回读 `0.1.19 / 199,239,208 bytes`，checksums 内容与 EXE / ZIP 一致。v0.1.18 五项资产仍完整保留，历史 tags 保留。
- 当前 Git / 版本：main 在本次记忆更新提交后可领先 v0.1.19 tag，不在该提交内预写自身未知 hash；Release 产品源码固定为 tag 指向的 `0b30064d399c69d7edc8f082d0ef27b40bc8c5d3`。历史正式 Releases / assets / tags 保留契约不变；不得移动、改写或删除任何已发布产品 tag / 正式 Release assets。
