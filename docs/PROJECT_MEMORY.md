...
> 当前未发布候选：版本已升至 `v0.1.21`，HB-047～HB-053 仍全部 `IN PROGRESS / UNVERIFIED`，HB-047 的政策 / 自定义分发继续 `ACCEPTED RISK`。用户已澄清“对手战绩无用”是要求修复，不是移除；任何删除对手战绩功能的方向均已完整撤回。HB-047 最终 dirty 候选已收口阶段化身份 / visibility、对手与历史参与者唯一关联、错误分类、瞬态历史重试和 Runtime 有界重试；最终审查无 P0 / P1。产品候选此前完整本地证据为 36 files / 350 passed + 1 skipped、typecheck、lint、source bridge / UI、OCR synthetic、真实 4K fixture（141ms）、icon、retention 与 diff-check 全过。candidate commit `c12b3a4` 的首轮 workflow_dispatch run `31799661557` / job `94764404770` 因 lifecycle fake BrowserWindow 缺少 `getNativeWindowHandle` 失败；修复后第二次 run `31802292958` / job `94772860036` 在 Windows `npm test` 取得 350 passed + 1 failed，唯一失败是 `window-manager-lifecycle` 的 champion dismissal 测试没有模拟 Windows `LeagueWindowObserver` visibility / placement gate，未进入 build / Release，不能解释为产品门禁回归。当前 test-only 补丁只把 observer gate 固定为 `setEnabled` no-op、`hasObservation / isClientVisible / isTargetPlaced=true`，隔离 dismissal generation 语义；只读终审无 P0 / P1。该补丁后的最新本地门禁为 36 files / 350 passed + 1 skipped、typecheck、lint、source bridge、source UI 与 diff-check 通过；仍待 commit / push / Windows retry。公开 Latest 仍为 v0.1.20，尚无 v0.1.21 tag / Release；不得预写 retry 结果或把本地门禁外推成 Windows、真实 WeGame / 用户同机已验证，也不得覆盖 v0.1.20 用户实机失败事实。
...
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 最终 dirty 候选已通过无 P0 / P1 的最终审查与完整本地全量；Windows、真实国服 WeGame 和用户同机仍未验证）
...
- 当前候选与验证：v0.1.20 正式 tag run `31780353884` / job `94704573925` 的 36 files / 306 passed + 1 skipped 只证明旧严格 `VISIBLE` 实现及当时 packaged / 发布链，不能验证本轮修复。当前 v0.1.21 最终 dirty 候选已实现上述阶段化可见性、唯一关联、错误分类、历史瞬态重试与 Runtime 有界重试；最终审查无 P0 / P1。本地完整 36 files / 350 passed + 1 skipped、typecheck、lint、source bridge / UI、OCR synthetic、真实 4K fixture（141ms）、icon、retention 与 diff-check 全过。Windows、真实 WeGame 和用户同机均未完成，因此仍不得写 `FIXED` / `VERIFIED`。历史端点、并发 2、2 MiB、Abort、Main-only 与 PUUID 不出 Main / 不日志 / 不进状态 / 不落盘边界未改变。
...
- 当前证据：最终审查无 P0 / P1；完整 36 files / 350 passed + 1 skipped、typecheck、lint、source bridge / UI、OCR synthetic、真实 4K fixture（141ms）、icon、retention 与 diff-check 全过。Windows、真实 WeGame 和用户同机均未完成；HB-047 当前为 `IN PROGRESS / UNVERIFIED`，不得写 `FIXED` / `VERIFIED`，政策 / 自定义分发继续 `ACCEPTED RISK`。固定 endpoint、Main-only 与 PUUID 不日志 / 不出 Main / 不进状态 / 不落盘边界不变。

2026-08-14 v0.1.21 / HB-048～HB-053 Windows 候选与首轮失败基线（尚未 tag / Release）：
...
- 本地 / Windows 证据：产品候选完整本地门禁曾达到 36 files / 350 passed + 1 skipped、typecheck、lint、source bridge / UI、OCR synthetic、真实 4K fixture（141ms）、retention、icon 与 `git diff --check` 全过，最终审查无 P0 / P1。candidate commit `c12b3a4` 首轮 run `31799661557` / job `94764404770` 因 lifecycle fake BrowserWindow 缺 `getNativeWindowHandle` 失败；修复后的第二次 run `31802292958` / job `94772860036` 在 Windows `npm test` 为 350 passed + 1 failed，唯一失败是 champion dismissal 测试未模拟 `LeagueWindowObserver` visibility / placement gate，未进入 build / Release，产品门禁无回归证据。当前 test-only 补丁仅固定 observer gate（`setEnabled` no-op，`hasObservation / isClientVisible / isTargetPlaced=true`）以隔离 dismissal generation 语义；只读终审无 P0 / P1，补丁后本地 36 files / 350 passed + 1 skipped、typecheck、lint、source bridge、source UI 与 diff-check 通过。仍待 commit / push / Windows retry；真实 WeGame 与用户同机均未完成，HB-048～HB-053 全部继续 `IN PROGRESS / UNVERIFIED`。
...
- HB-047 当前为 `IN PROGRESS / UNVERIFIED`，政策 / 自定义分发为 `ACCEPTED RISK`：用户要求修复而非移除对手战绩，删除方向已完整撤回。v0.1.21 最终 dirty 候选实现 selecting / launching=champ-select、active=gameflow 的阶段 visibility，要求本方 / team 与五个对手身份唯一且 visibility 拒绝优先；fulfilled null / error envelope 分类、历史 identity↔participant 唯一 join / 单 participant legacy fallback、500 / 429 一次重试与 404 / schema 不重试均已收口。Runtime 仅对 source / team incomplete 或 transport Abort 做 3 / 5 / 10 / 15 / 15 秒有界重试，hidden / ambiguous 不重试，成功停止、耗尽手动。最终审查无 P0 / P1；36 files / 350 passed + 1 skipped 与完整本地门禁全过，但 Windows / 真实 WeGame / 用户同机尚未完成，不得标 `FIXED` / `VERIFIED`。
