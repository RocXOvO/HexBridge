...
> 最后更新：2026-08-14
> 当前基线：公开最新正式 Release 为 [v0.1.23](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.23)，Release ID `370625932`，publishedAt `2026-08-14T14:25:52Z`，public、Latest、non-draft、non-prerelease。annotated tag object `e53cb90a02c43010c6cfee68758d525a91b108b6` 解引用到产品 / 标签提交 `06ca3f5307133c86bc0f8b5c5cbc8eed76c16d7d`；tag run `31808909709` / job `94794460360` 于 `2026-08-14T14:19:13Z`～`14:26:03Z` success，约 6m50s。Windows 36 files / 359 passed 及全部候选、正式 Release / channel / public 门禁通过；public v2 channel 已独立核验 `version=0.1.23`、EXE size `199254309`，public packaged check 返回 `channelVersion=0.1.23 / updateAvailable=false`。v0.1.11～v0.1.23 各保留五项正式资产，legacy 根 channel 仍冻结为 0.1.14。
...
> 当前未发布候选：在公开 Latest v0.1.23 之上，v0.1.24 仅实现 HB-054“队友 + 对手近期状态与点击头像脱敏明细”。candidate commit `24a094b4c4dd1b1570b54cdc78d9845e2a0e2d5b` 已 push main；workflow_dispatch run `31813165279` / job `94808409628` success，Windows 36 files / 370 passed 及全部候选门禁通过，tag-only Release / channel / public 步骤按预期 skip。尚未 tag / Release；真实 WeGame / 用户同机仍未验证，HB-047 / HB-054 继续 `IN PROGRESS / UNVERIFIED`，政策 / 自定义分发继续 `ACCEPTED RISK`；不得预写 tag / Release 结果。
...
- 交付节奏：用户要求每完成一项独立目标就逐项发布一个小版本。只有该项目标完成对应审查、自动化与必要 Windows 候选门禁后才可进入发布流程；不得预先分配版本号、捆绑宣称未完成目标、跳过验证或预写 tag / Release 结果。
- 队友 / 对手近期状态与“上等马 / 中等马 / 下等马”只允许作为默认关闭、仅本机的个人实验；当前 dirty 候选不再把历史样本限定为 Mayhem，但只有 HexBridge 已建立有效比赛上下文时才能进入。身份来源随阶段收口：`selecting / launching` 只信 champ-select，`active` 只信 gameflow；本方必须唯一、两组身份必须跨组唯一，任一原始 team 超过 5 人时全局 fail closed。4 位队友和 5 位对手各自 all-or-nothing，单组失败只禁用该组，组间允许 partial。不得隐藏身份绕过、LCU 写入、注入、上传、记录或持久化 PUUID / 原始响应 / 完整战绩；少于 12 个有效样本不得评分 / 分档，来源不成立时保持不可用。该实现不表示 Riot 官方支持或适合公开分发；身份可见性、竞争优势、自定义对局历史与玩家价值分档继续由 HB-047 / HB-054 按 `ACCEPTED RISK` 明示保留。
...
- 实验性对手战绩目标：默认关闭、仅本机、受支持海克斯队列 `{2400, 3270}`，明确包含用户实际使用的自定义 3270。`selecting / launching` 必须从 champ-select 取得身份，只接受显式 `VISIBLE` 或显式空字符串；`active` 必须从 gameflow 唯一定位本方并取得对方恰好 5 个唯一合法 PUUID，此阶段允许可见性字段缺失 / `null` 或 `VISIBLE`，拒绝显式空字符串、`HIDDEN`、`UNKNOWN` 与其他未知值。只有对应阶段门禁成立才可由 Main 调用固定 current-summoner 与逐 PUUID match-history GET。历史响应上限 2 MiB、并发 2、可 Abort；只取最近可用 LoL 样本，不限定 Mayhem，少于 8 局不评分 / 不分档。PUUID、原始响应和完整历史不得离开 Main、写日志、落盘或上传，非 Main 状态必须剥离该业务状态；来源 / 样本不成立时保持不可用。该功能当前是修复中的 dirty 候选而非删除候选；它不得冒充官方支持，扩大或自定义对局分发仍有明确政策风险并按 `ACCEPTED RISK` 管理，见 HB-047。
...
- `src/main/window-manager.ts`：主窗口、选人伴随窗、96px 小型 augment 推荐窗与校准窗口；augment 窗保持透明、点击穿透、不可聚焦，严格 3/3 `augmentId` 后先发送 compact DTO 再仅于 active + 游戏前台显示。
- `src/main/league-window-observer.ts`：Windows-only 单个持久 PowerShell / Win32 观察器；DWM extended bounds / PMv2、前台客户端 PID 偏好、350ms 跟随、异常指数退避与 stale child 守卫。选人期跟随 `LeagueClientUx`，对局期提供游戏前台状态；不得扩展为客户端内容读取或输入注入。
...
- `src/renderer/`：实时助手、英雄榜、设置、诊断、选人伴随窗、校准界面与受限 `#augment` route；`additionalArguments` 决定 preload 只暴露 `hexbridgeOverlay` compact bridge，`AugmentOverlay.vue` 不接收完整 RuntimeState 或主业务 API，主实时助手仍保留本局最后可靠结果。
...
- `/lol-summoner/v1/current-summoner`（仅限 Main 内 HB-047 本机实验与 HB-053 中途启动英雄恢复）
- `/lol-match-history/v1/products/lol/{puuid}/matches?begIndex=0&endIndex=19`（仅接受 `[A-Za-z0-9_-]{20,120}` 的 PUUID 路径段和这一固定 query）

HB-047 / HB-054 对 allowlist 的扩展只适用于默认关闭、仅本机的 `ACCEPTED RISK` 个人实验。进入历史请求前必须满足阶段化来源：`selecting / launching` 只使用 champ-select，显式 `VISIBLE` 或显式空字符串可接受，字段缺失、`HIDDEN` 或未知值拒绝；`active` 只使用 gameflow，可见性缺失 / `null` / `VISIBLE` 可接受，显式空字符串、`HIDDEN`、`UNKNOWN` 或其他未知值拒绝。本方身份与归组必须恰好唯一，队友 / 对手必须跨组唯一，任一原始 team 超过 5 人时全局 fail closed；4 位队友与 5 位对手分别 all-or-nothing，单组失败只禁用该组，因此组间可以 partial。Main 先读取固定 current-summoner，再对合格身份调用固定 `begIndex=0&endIndex=19` 历史端点；最多取 20 场，少于 12 个有效样本不评分 / 分档。跨旧批次的全局历史请求并发最多 2，单响应硬上限 2 MiB，支持 timeout / Abort，瞬态失败最多重试一次；禁用、换局、transport / credential 变化、退出或新批次取代旧批次时必须取消请求并清空明细缓存。不得接受任意 endpoint、query、索引范围或 Renderer 提供的 PUUID，不得用屏幕抓取、客户端注入或不明第三方接口补齐身份；所有请求继续为 `GET`，不得新增 LCU 写调用。

HB-053 只复用固定 current-summoner GET，并且只在 `active` 阶段于 Main 内把其 PUUID 与 gameflow team 做唯一匹配以恢复本英雄；匹配不唯一或来源不成立时不得猜测 champion。失败后的重试边界固定为 5000ms：4999ms 不得重试，达到 5000ms 才可再次请求。该路径不得调用逐 PUUID match-history、不得把 PUUID 加入 `ChampSelectSnapshot` / Renderer / IPC 状态或日志 / 磁盘；目标回归同时要求 snapshot 与 logger 均不含 PUUID。

非 Mayhem 隔离契约：只要当前队列不属于 `{2400, 3270}`，Runtime / Renderer 就不得暴露上一局或当前观察中的 hero、bench、candidates 或 build；即使跨队列英雄 ID 相同，也必须清除旧 champion detail、augment overlay 与存在性 monitor，不得按 same-hero 复用。current-summoner 的私有 PUUID 只能在 Main 的 active 有界内存中存在，`active→none` 或既有 generation 换代时必须同步清理，不得残留到下一上下文。非 Mayhem 字段隔离与 same-hero 跨队列清理两项审查未发现 P0 / P1；真实 WeGame 跨队列链仍未验证。
...
### 3.2.1 可整合数据源审计（多数方案未实现；HB-047 窄实验已随 v0.1.20 发布）
...
4. **本地 LCU Match History（默认关闭的 HB-047 / HB-054 个人实验，v0.1.20 首次接入，当前 dirty 候选已扩展）**
   - 固定只读 GET 为 `/lol-summoner/v1/current-summoner` 与 `/lol-match-history/v1/products/lol/{puuid}/matches?begIndex=0&endIndex=19`。选人 / 启动阶段只使用 champ-select，active 只使用 gameflow；唯一 self、跨组唯一和 team 上限先做全局门禁，再让 4 位队友 / 5 位对手分别 all-or-nothing、组间可 partial。Main 内跨旧 / 新批次的历史请求总并发最多 2，单响应 2 MiB 上限并支持 Abort / timeout，瞬态失败最多重试一次。
   ...
   - 只在 Main 内存中把最近最多 20 场可用 LoL 对局归一化，不限定 Mayhem；少于 12 个有效样本不计算 rating / 三档。PUUID、玩家名、participant、gameId、原始响应、逐局时间戳与完整历史不得进入 Renderer、日志、磁盘、遥测、上传或跨设备同步；关闭实验、离局、transport / credential 变化、换代、退出或请求失败后不得持久保留。
...
审计后的整合原则：聚合统计继续来自文档化且获授权的数据接口；静态海克斯元数据可评估 pin patch 的 CommunityDragon；Riot / Meraki 只补静态资产。LCU Match History 只允许 HB-047 / HB-054 当前这条默认关闭、本机、身份明确可见、严格限流且不持久化的 `ACCEPTED RISK` 个人实验；样本可来自最近可用 LoL 对局而不限定 Mayhem，但查询资格仍依赖当前有效比赛上下文和上述阶段 / 分组门禁。它不能被描述为官方数据能力、生产稳定源或公开分发已获批准。Live Client Data API 若接入仍只限当前局已公开最小字段；Wiki 只在满足 CC BY-SA 许可隔离时用于交叉校验；不使用私有网页抓取填补数据空缺。
...
- `getState()` / `onStateChanged(callback)`：只有 Main sender 可取得经聚合的队友 / 对手 summary DTO；选人伴随窗、augment、校准等非 Main 状态必须剥离该字段或返回 disabled / empty。summary 只含 relation、slot、championId、清洗后的样本 / 评分以及 generation 绑定的随机 24 字符 opaqueKey；任何 Renderer 都不得取得 PUUID、玩家名、participant、gameId、原始历史、逐局时间戳、路径或可重放查询参数。
...
- `retryOpponentScout()`：无参数、只允许 Main sender；只重新触发 Main 内相同的 stage / visibility / self / team / generation 门禁，不能绕过资格条件或由 Renderer 指定玩家。
- `getScoutPlayerDetails(opaqueKey, matchGeneration)`：只允许 Main sender；opaqueKey 必须是 24 字符 allowlist 格式，并同时命中当前 generation、当前 summary membership 与 Main 内存缓存，否则 fail closed。Renderer 不能提交 PUUID、endpoint 或 query；成功 DTO 只含 relation / slot、championId，以及最多 20 场清洗后的英雄、胜负、K / D / A 和分钟数。
...
- v0.1.12 默认关闭自动 OCR；v0.1.20 候选只有本局为 `active`，且“Main 可见、未最小化并聚焦”或“游戏内推荐已启用且 League 游戏窗口处于前台”至少一个可见表面成立时才调度自动扫描。用户显式开启后每 2 秒只允许一个自动扫描任务，先做最大宽 960px 的低分辨率标题 ROI 门控，信号命中后才进入 full OCR；`busy` 时拒绝自动重入。手动按钮 / 当前配置快捷键只触发一次有界捕获，不经过周期门控，并可为取得优先级等待在途自动任务最多 1.5 秒。
- 先对三块裁切做低成本灰度信号门控，至少两块命中才启动 OCR。
- 三块按左 / 中 / 右串行识别，中文标准化后精确或模糊匹配；置信度阈值不得低于 90%。
- 三张全部可靠识别才自动展示；组合去重。v0.1.20 候选在同一组三卡成功后继续用 960px 标题 ROI 门控生成三个标题指纹，并以单槽变化比例的最大值判断组合变化，避免一张刷新被其他两槽平均稀释；候选变化连续两帧稳定才触发一次新的 full OCR，以覆盖刷新卡牌并抑制单帧动画 / 亮度噪声。该变化不得清除 HB-034 规定的上一组可靠展示结果，也不得让 full OCR 无界重复。当前配置的手动快捷键只强制重新截图 / 识别，不发送键鼠事件；其原子注册与 Windows 游戏前台未验证边界见 HB-025。
- 识别结果呈现契约按用户后续需求收口：海克斯三卡结果与推荐继续作为实时助手页的对局内容，同时允许 HB-046 的 96px 小型透明推荐窗作为窄例外。v0.1.14 已移除的全屏 / 顶部大块黑色浮窗、黑色底板与遮挡式宿主永久禁止恢复；小窗必须点击穿透、不可聚焦，严格三槽均有 `augmentId` 后先收 compact DTO 再于 active + 游戏前台显示。
- 自动扫描只在目标本局上下文为 `active + queueId ∈ {2400, 3270}`、自动 OCR 已显式开启、Runtime 未 stopping，并且 Main 聚焦表面或游戏前台推荐表面至少一个成立时运行；LCU transport 交接本身不能否定仍有效的 active generation。离局 / generation 或 champion 变化、Main 失焦且游戏不在前台、关闭自动 OCR 或 stopping 必须停表；在途扫描结束后再次以 scan epoch + generation + champion + stopping 原子校验，旧结果不得进入新局或重新显示过期小窗。
...
- HB-047 / HB-053 的 PUUID、current-summoner 原始载荷、逐玩家 match-history 响应与完整历史只能存在于 Main 的有界内存缓存 / 在途请求；不得写持久缓存 / 日志 / 截图 / 磁盘、进入 Renderer / overlay、上传或遥测。关闭功能、transport / authority 断开或变化、离开支持阶段、generation 变化或 Abort 后必须取消 / 丢弃旧结果；UI 只消费不含身份标识和原始历史的有限聚合 DTO。
...
- 状态：`IN PROGRESS`（v0.1.19 Windows 候选已通过，尚未用户验收）
...
- 验收标准：覆盖当前版=public latest、发现更高 stable、prerelease / draft / downgrade、检查中、离线 / 403 / 404 / TLS、metadata 无效和更新被对局阻止；source / packaged UI 均断言只有可应用更新状态显示按钮，键盘焦点与无障碍名称随显隐正确。当前已有本地测试、预览与 Windows packaged UI 证据，但没有用户验收或 Release，不能标 `FIXED` / `VERIFIED`。
...
- 验收标准：视觉快照覆盖 missing / validating / ready / stale / unauthorized / limited / offline / error、长中文、100%～150% DPI、键盘操作与 reduced-motion；真实 packaged 需验证 ready、替换失败仍保留旧 Key、清除和固定外链。当前已有本地测试 / 预览与 Windows packaged UI 证据，但没有真实 Key / 用户验收或 Release，不能标 `FIXED` / `VERIFIED`。
...
- 验收标准：source / packaged 视觉快照覆盖三档、focus / visibility、InProgress 和 reduced-motion；真实 4K Windows 游戏对照记录 HexBridge CPU / GPU、FPS / frametime、长任务、RAF / timer / observer 数量与窗口切换恢复。Windows runner 已通过功能烟测，但没有用户同机性能或 Release 证据，仍不得写 `FIXED` / `VERIFIED`。

### HB-044 页面切换滚动条显隐导致布局抖动，现有动效需柔化

- 严重度：中（不阻断功能，但页面切换产生明显横向位移和生硬感）
- 状态：`FIXED / UNVERIFIED`（样式与 source UI 窄门禁通过，尚无 Windows packaged / 用户视觉验收）
- 用户症状：从其他页面切换到实时助手时，纵向滚动条会出现或消失，页面内容随之抖动；用户同时要求当前导航、页面进入与内容更新动效更柔和。
- 当前实现：内容 stage 固定 `overflow-y: scroll` 并启用 stable scrollbar gutter，使长短页面和 Vue `out-in` 中间态持续预留纵向滚动空间；页面进退场改为 4px 轻位移与透明度淡入，不再使用 scale。没有新增持续动画、重型滤镜、 timer、RAF、observer 或运行时依赖；`eco`、`InProgress`、hidden / minimized / unfocused 与 reduced-motion 的既有静态降级保持不变。
- 当前证据：source UI smoke 循环进入实时助手与诊断页，二者内容区 `clientWidth` 均为 953px；最终独立审查无已知 P0 / P1，v0.1.20 本地全链为 34 files / 293 passed + 1 skipped，typecheck、lint、source UI / bridge / preload / retention 与 diff-check 通过。该证据验证了受控 source viewport 的宽度稳定，不等于 Windows 系统滚动条、不同缩放或用户视觉已经验证。
- 剩余验收：在 Windows 主窗口最小 / 默认 / 最大尺寸和 100%～150% DPI 下循环切换实时助手、英雄榜、设置和诊断，确认无横向跳动、双滚动条、焦点 / 滚动位置异常，并覆盖 reduced-motion。用户同机确认前不得标 `VERIFIED`。

### HB-045 ChampSelect 伴随窗未持续跟随 League 客户端窗口

- 严重度：中高（移动或缩放 League 客户端后伴随窗与选人界面脱离）
- 状态：`FIXED / UNVERIFIED`（Windows synthetic native-window follow 窄范围 `VERIFIED`；真实 WeGame `LeagueClientUx`、多屏 / 混合 DPI 与最小化仍未验证，总体不得升级）
- 用户症状：此前伴随窗只在显示时取得一次 League 客户端坐标；之后移动客户端，伴随窗仍留在旧位置。
- 当前实现：新增单个持久 PowerShell / Win32 观察器；以 PMv2 DPI awareness 优先读取 DWM extended frame bounds，失败才回退 `GetWindowRect`。只在受支持选人期跟随，每 350ms 枚举可见、非最小化的 `LeagueClientUx` 主窗口，优先当前前台或最近前台客户端 PID，再以面积稳定决策，并用 `SetWindowPos` 只移动 HexBridge 伴随窗。相同 target handle 与 follow 模式复用已有子进程，不重复 spawn；观察器异常终止按 1.5s 起、最高 30s 指数退避重启，旧 child 的迟到输出 / close / error 受实例与配置守卫，不得污染新 observer。League 客户端最小化 / 不可见时隐藏伴随窗，离开选人、应用退出或目标销毁时停止。实现不注入 League 进程、不 Hook 输入、不发送键鼠、不读取窗口内容，也不把句柄、路径或玩家标识写入日志。
- 当前 Windows synthetic native-window follow 证据：commit `b0bbc1605d89c61f8a538639f280ae470ab8a33c` 新增仅由 smoke 显式启用的原生 WinForms `Fake LeagueClientUx.exe`，每 900ms 在两个位置间移动；packaged bridge 使用完整生产 `LeagueWindowObserver` 跟随一个隐藏 HexBridge BrowserWindow，并严格要求 `getBounds()` 跨两个目标位置发生变化。正常应用运行路径不创建或触发 fake；外层 smoke 负责 C# 编译、ready 同步、30s deadline、进程树清理，并只在严格 `result.windowObserverFollow` 成立时通过。最终独立审查无已知 P0 / P1；workflow_dispatch run `31777136598` / job `94694840505` 基于 `b0bbc16` 约 5m16s success，packaged bridge 原生移动窗口持续跟随及完整候选门禁全过。该窄证据足以验证生产 observer→Win32 窗口移动链，但 fake 不是 WeGame。
- 剩余验收：真实 Windows packaged 中覆盖单 / 多个真实 `LeagueClientUx`、连续移动 / 缩放、最小化 / 恢复、跨屏、负坐标、100%～150% 及混合 DPI、关闭与下一次选人，确认不抢焦点、无重复 PowerShell、无窗口重建风暴、退出无残留且 CPU 有界。Windows synthetic 窄证据不得外推为这些真实场景，总体继续 `FIXED / UNVERIFIED`。

### HB-046 游戏内三卡下方小型推荐窗与刷新后实时更新

- 严重度：高（用户在实际选择时无法就地查看推荐；错误恢复旧覆盖层又会遮挡游戏或影响性能）
- 状态：`FIXED / UNVERIFIED`（小窗、compact IPC 与刷新门控已实现并通过 source 窄门禁，尚无真实 Windows 4K 游戏验收）
- 用户目标：自动 OCR 可靠识别左 / 中 / 右三张卡后，在三张卡牌下方显示与卡位对齐的小型推荐信息；玩家刷新卡牌后，新组合可靠识别完成即在原位更新，不要求切回主窗口。选择仍由用户完成，软件不得自动点击或发送输入。
- 当前窗口实现：WindowManager 创建固定高度 96px、透明背景、点击穿透、不可聚焦、不可移动 / 缩放、skip-taskbar 的 augment BrowserWindow；受控 `additionalArguments` 决定 preload route，augment 只获得独立 `hexbridgeOverlay.onChanged`，不能取得主业务桥。按真实整卡校准矩形计算三张卡牌的独立 columns 与下方 bounds；非法、乱序或仍像 OCR 标题带的 geometry 回退审计过的默认整卡矩形。compact DTO 仅含 `slots + layout + message`；严格要求三槽均有 `augmentId`，并且先 send 后 `showInactive`，避免首帧空窗。只在 supported match `active` 且 Windows 观察器确认 League 游戏前台时显示；其他状态隐藏。自动 OCR 仍默认关闭，用户未显式开启时不会因小窗增加周期截图。
- 当前刷新实现：首次 full OCR 成功后继续使用 960px 标题 ROI 门控计算三个标题指纹；变化距离取三个槽位中单槽变化比例最大值，使单卡刷新不会被平均稀释。候选指纹连续两帧稳定才触发一次新的 full OCR，以更新刷新后的卡牌并过滤单帧动画 / 亮度噪声。同组合和短暂 miss 继续保留上一可靠结果；部分 OCR、缺任一 `augmentId` 或猜测名称不进入小窗，离局 / 新 generation / 显式禁用按既有边界隐藏。自动调度还要求 Main 可见未最小化且聚焦，或游戏内推荐启用且 League 游戏在前台；二者都不成立即停表。
- 与 HB-029 的兼容边界：v0.1.14 移除的全屏 / 顶部 augment BrowserWindow、黑色底板和大块覆盖没有恢复。当前窗只覆盖卡牌下方推荐带，不使用全屏透明父窗，不阻挡鼠标，不 show / focus / restore 主窗口，也没有持续发光或循环动画；实时助手继续保留本局最后可靠结果。
- 当前证据：layout 测试覆盖真实整卡 columns、默认 / 校准矩形卡下 bounds 与异常 geometry fallback；Renderer / preload 契约覆盖 additionalArguments bridge 隔离、compact DTO 与严格 3/3 门禁。source UI smoke 验证 augment route 视口为 960×96、背景透明、compact bridge 正常，实时助手 / 诊断 stage 宽均为 953px；OCR synthetic 和真实 4K fixture 134ms 且三块 detected。最终独立审查无已知 P0 / P1，v0.1.20 本地全链为 34 files / 293 passed + 1 skipped，typecheck、lint、source UI / bridge / preload / retention 与 diff-check 通过。尚未在 Windows 真实游戏证明前台检测、点击穿透、DPI 布局、刷新换代或性能。
- 剩余验收：1080p / 2K / 4K、100%～150% DPI、多显示器和真实海克斯刷新中确认不黑屏、不遮卡、不抢焦点、点击穿透、原位更新且无明显 FPS / frametime 峰值；另覆盖游戏失焦 / 恢复、手动 OCR、自动 OCR 显式开关和退出销毁。完成用户实机前不得标 `VERIFIED`。

### HB-047 实验性本地只读对手战绩与分档评分

- 严重度：高（涉及玩家身份、战绩隐私、未文档化端点稳定性和 Riot 产品政策；错误评分会直接误导）
- 状态：`IN PROGRESS / UNVERIFIED`。v0.1.21 的仅对手版本已正式发布；v0.1.24 Windows 候选已扩展为队友 + 对手分组和点击头像脱敏明细，最终审查与 workflow_dispatch 无 P0 / P1，但尚未 tag / Release，真实国服 WeGame、身份 / 历史端点与用户同机仍未验证。
- 用户决策：用户已澄清“对手战绩无用”是要求修复现有功能，而不是移除功能；此前任何删除 / 移除对手战绩的方向均已完整撤回，默认关闭、本机实验入口、Main-only 查询和隐私边界继续保留。
- 政策 / 分发边界：`ACCEPTED RISK`。用户明确选择保留默认关闭、仅本机、当前受支持海克斯比赛上下文 `{2400, 3270}` 的个人实验；历史样本可取最近可用 LoL 对局而不限定 Mayhem。这不构成 Riot 批准。Riot General Policies 2025-03-11 与 LoL policies 对不公平优势、替代官方技能评级、去匿名化隐藏玩家及未经 opt-in 展示自定义对局历史仍构成明确风险，因此不得据本地实现宣称适合公开或商业分发。
- 启用与身份门禁：设置默认 `opponentScouting=false`，迁移也保持关闭。`selecting / launching` 只信 champ-select；`active` 只信 gameflow。current-summoner 必须唯一落入一个 team，所有有效身份必须跨两组唯一，任一原始 team 超过 5 人立即全局 fail closed。去掉 self 后，4 位队友与 5 位对手分别 all-or-nothing；一组不可见 / 不完整只禁用该组，另一组合格时允许 partial。champ-select 仅接受显式 `VISIBLE` / 显式空字符串，active 接受可见性字段缺失 / `null` / `VISIBLE`；其他标记拒绝。不得用 OCR、进程注入、内存读取、权限绕过或私有第三方服务补齐身份。
- LCU 与资源边界：Main 只调用固定 `/lol-summoner/v1/current-summoner` 和 `/lol-match-history/v1/products/lol/{puuid}/matches?begIndex=0&endIndex=19`。PUUID 路径段固定校验，Renderer 不能传 endpoint、PUUID 或 query。跨被取消旧批次与新批次的全局历史请求并发始终最多 2；单响应 2 MiB 上限，支持 timeout / Abort，瞬态失败最多自动重试一次，404、schema / 结构错误和其他确定性失败不重试。禁用、换局、transport / credential 变化、退出或新批次取代旧批次时必须取消请求并清空 summary 明细缓存；不得引入 LCU 写请求。
- 样本与评分口径：每位玩家最多消费 20 场最近可用 LoL 对局，不限定 Mayhem；过滤无效、过短或缺关键统计的对局。目标玩家必须在 identity 与 participant 两侧唯一 join；缺 PUUID 的 legacy identity 只允许单 participant 有界 fallback，畸形、重复或歧义 fail closed。有效样本少于 12 局不得计算 rating 或“上 / 中 / 下等马”，不得以 0、默认档或猜测补齐。
- 隐私 / IPC 边界：PUUID、玩家名、participant、gameId、current-summoner / history 原始载荷、逐局时间戳、路径和完整历史只可在 Main 有界请求处理中短暂存在，不得进入 Renderer、日志、配置、磁盘、截图、遥测或上传。每个可用玩家只向主窗口发 generation 绑定的随机 24 字符 opaqueKey 与脱敏 summary；点击头像只允许 Main sender，并同时校验 key 格式、当前 generation 和 current membership。Main 返回的明细只含英雄、胜负、K / D / A 与分钟数；非 Main state 必须剥离整个 scout DTO。
- 生命周期 / 交互契约：禁用、generation / stage 换代、transport / credential 变化、退出和新批次开始必须 Abort 并清空请求 / opaqueKey / 明细缓存；selecting / launching 的 partial 进入 active 后必须改用 gameflow 重新查询。头像明细对话框必须初始聚焦、Tab / Shift+Tab 焦点陷阱、Escape / 背景 / 关闭按钮退出，并在关闭或请求失败后把焦点恢复到仍存在的触发头像。
- active 重试契约：首个 active 身份尚未就绪时，不得让 generation / stage `attemptKey` 永久锁死查询。仅 source / team incomplete 或 transport Abort 可按既有 3 / 5 / 10 / 15 / 15 秒做最多五次有界自动重试；hidden、ambiguous、visibility 拒绝、404、schema 错误或其他确定性失败不得轮询。成功、partial / ready、离局、禁用、generation / stage 变化或停止后必须停止 / 清理；选人阶段的 partial 不轮询历史，进入 active 后按新来源重查。
- 当前实现与验证：v0.1.24 Windows 候选实现以上队友 / 对手、partial、20 场 / 12 样本、跨批次并发、opaqueKey 明细与焦点管理；README 已同步，终审无 P0 / P1。本地 36 files / 369 passed + 1 skipped 与全链通过；Windows run `31813165279` 为 36 files / 370 passed，packaged UI / bridge / differential / artifact 全过。尚未 tag / Release，公开 Latest 仍为 v0.1.23；真实 WeGame、LCU 历史端点和用户同机未验证，因此不得写 `FIXED / VERIFIED`。
- 剩余验收：Windows packaged / 真实 WeGame 覆盖 2400 和自定义 3270 的 selecting→launching→active、两组完整 / 单组 partial / 全局歧义、raw team >5、跨组重复、所有 visibility 组合、partial→active 重查、2 MiB / 并发 2 / timeout / Abort / 瞬态重试、禁用 / 换局 / transport / credential / 退出清理、少于 12 局不分档、点击明细焦点与失效 key，以及 Renderer / 日志 / 磁盘无身份 / 原始历史泄漏。扩大分发前仍须重新做 Riot 政策与玩家 opt-in 审计。

### HB-048 自定义 3270 退出后比赛上下文和选人伴随窗未清理

- 严重度：高（离开房间后继续显示已失效英雄与窗口，会把上一局状态误当作当前状态）
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 与 v0.1.22 正式 Windows tag 全链已通过；真实 WeGame / 自定义 3270 退出链与用户同机仍未通过）
...
- 当前实现：v0.1.21 正式版统一租约为 `active=12h`、`committed=10m`、`weak=15s`；空 ChampSelect、`None` 或 partial observation 不得续期，只有正向 `GAME_STARTING`、`gameClient`、独立游戏进程或可靠 augment 证据才允许进入对应长租，并把伴随窗手动关闭绑定到当前 generation。v0.1.22 正式版又要求非 Mayhem 不暴露 hero / bench / candidates / build、same-hero 跨队列清 detail / overlay / monitor，以及 private current-summoner PUUID 在 `active→none` / generation 换代清理。非 Mayhem 字段隔离与 same-hero 跨队列清理两项审查无 P0 / P1，但该组实现仍未证明覆盖用户真实 3270 / 跨队列链，状态不得升级。
...
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 与 v0.1.22 正式 Windows tag 全链已通过，后者含 fake `LeagueClientUx` 生产跟随窄证据；真实 WeGame、多屏 / DPI 与用户同机仍未通过）
...
- 当前 v0.1.22 正式实现：authority PID 必须硬过滤；已验证 HWND 走 80ms 快速矩形读取 / `SetWindowPos`，完整进程重发现最多 1Hz，没有 follow 资格时仍维持 350ms 守卫，并改为 0px 贴边。最后一项 P1 修正后最终审查无 P0 / P1，本地完整门禁与 Windows packaged fake-window 跟随已过；真实 WeGame、多窗口或混合 DPI 均未完成，不得把方案或 fake window smoke 写成修复完成。

### HB-050 游戏内刷新海克斯后 OCR 与推荐更新过慢

- 严重度：高（刷新后的实际选择窗口有限，迟到推荐失去作用）
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 与 v0.1.22 正式 Windows tag 全链已通过；真实游戏刷新时序、性能与用户同机仍未通过）
...
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 与 v0.1.22 正式 Windows tag 全链已通过；真实上游数据对应性与用户同机仍未通过）
...
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 与 v0.1.22 正式 Windows tag 全链已通过；真实三卡选择 / 刷新、性能与用户同机仍未通过）
...
- 当前 v0.1.21 正式实现：主窗口继续保留最后可靠 slots；96px compact 只在卡面可靠存在时显示，并在失焦、功能禁用、刷新开始、连续两次可靠 absence 或显示满 45 秒时关闭。`autoOcr=false` 的手动成功路径只启动上述有界低成本存在性监测，不转为持续 full OCR。该超时 / absence 策略仍需真实三卡选择、刷新与性能验收。

### HB-053 中途启动 HexBridge 时无法恢复本局英雄

- 严重度：高（应用在对局中途启动时若没有当前英雄，OCR 推荐和英雄专属数据无法建立可靠上下文）
- 状态：`IN PROGRESS / UNVERIFIED`（v0.1.21 与 v0.1.22 正式 Windows tag 全链已通过；真实 WeGame active 中途启动与用户同机仍未通过）
- 当前实现：Main 只读缓存 current-summoner PUUID，并且只在 `active` 阶段与 gameflow team 做唯一匹配以恢复当前英雄；匹配不唯一或来源不成立时不得恢复。失败退避固定为 5000ms；目标门禁覆盖首次 `null` 后 4999ms 不重试、5000ms 成功，并断言 snapshot / logger 均无 PUUID。v0.1.22 正式版进一步在 `active→none` 与既有 generation 换代时清理 private PUUID；PUUID 只可存在于 Main 有界内存，不得写日志、Renderer、IPC 业务状态、缓存、配置或磁盘。
...
### HB-054 队友与对手战绩总览及头像详情

- 严重度：中高（属于用户明确提出的战绩能力扩展，同时涉及身份可见性、隐私与自定义分发政策风险）
- 状态：`IN PROGRESS / UNVERIFIED`；v0.1.24 Windows 候选已实现并通过终审 / 本地 / workflow_dispatch 门禁，但尚未 tag / Release，也没有真实 WeGame 用户同机证据，不得写 `FIXED / VERIFIED`。政策 / 自定义分发继续 `ACCEPTED RISK`。
- 当前实现：默认关闭，只在 Main 内按 HB-047 阶段化身份门禁分别构造 4 位队友和 5 位对手。任一组只有全员合格才查询 / 展示，但另一组可独立成功形成 partial；任一 raw team >5、本方不唯一或跨组身份重复时全局拒绝。每位合格玩家固定读取最多 20 场，少于 12 场不评分 / 分档。
- 头像明细：Renderer 只持有 relation / slot、championId、脱敏 summary 与 generation 绑定的随机 24 字符 opaqueKey。点击头像经 Main-only IPC 同时校验 key、generation 和 current membership，只返回英雄、胜负、K / D / A、分钟；PUUID、玩家名、participant、gameId、原始载荷、逐局时间戳和路径不进入 Renderer / 日志 / 磁盘。禁用、换局、transport / credential 变化、退出或新批次会取消请求并清理 key / 缓存。
- 交互：对话框打开后初始聚焦关闭按钮，Tab / Shift+Tab 受焦点陷阱约束，Escape / 背景 / 关闭按钮可退出；请求失败或关闭后恢复到仍连接的触发头像。selecting / launching 的 partial 进入 active 后必须用 gameflow 重查。
- 当前证据 / 验收：README 已同步，最终审查无 P0 / P1；本地 36 files / 369 passed + 1 skipped、Windows 36 files / 370 passed，typecheck、lint、source / packaged bridge / UI、differential、artifact 与 diff-check 全过。仍须真实 WeGame 验证完整 / partial / 全局拒绝、20 场 / 12 样本、并发 / Abort / 清理、点击失效 key、焦点恢复、性能和隐私泄漏；未完成前状态不升级。

### HB-055 真实对局内 96px 提示 / 推荐窗缺失

- 严重度：高（实际三卡出现时没有游戏内提示，会使核心对局推荐不可用）
- 状态：`IN PROGRESS / UNVERIFIED`；v0.1.23 正式版已定位并修复自动化可复现根因且 Windows tag 全链通过，但尚无真实 WeGame 用户同机证据，不得写 `FIXED / VERIFIED`。
...
- v0.1.23 正式实现：前台丢失只暂停监测并保留 committed fingerprint / context / slots / deadline，WindowManager 临时隐藏小窗；在原 deadline 内回到前台时直接重新显示并恢复 cheap probe，不触发 full OCR。独立 45 秒 expiry 以零截图主动撤销；连续两次 reliable absence、refresh、功能 disable、auto OCR 开启、terminal、generation 换代或 stop 均撤销。`autoOcr=true→false` 且已有可靠 3 / 3 结果时建立新的 45 秒 cheap monitor。
...
- 严重度：中（当前英雄背景过度暗化 / 模糊会削弱沉浸感，但不能以牺牲前景信息可读性或对局性能换取清晰度）
- 状态：`IN PROGRESS（REPORTED / UNDIAGNOSED）`；仅登记视觉目标和验收边界，尚未审计当前滤镜参数、对比度或实现路径，不得预写修复。
- 用户目标：选择英雄后，实时助手主背景中的英雄原画应比当前更清晰、更容易辨认；英雄名称、Tier / 胜率、出装与推荐等前景信息仍须保持稳定可读，不能通过简单移除所有遮罩造成文字失去对比。
- 性能 / 可访问性边界：继续服从自动视觉档以及 cinematic / balanced / eco、hidden / minimized、InProgress 和 `prefers-reduced-motion` 策略；不得恢复持续重绘、重型模糊、粒子或对局内环境动画，不得增加明显 CPU / GPU、FPS 或 frametime 峰值。
- 验收契约：覆盖 100% / 125% / 150% DPI、长中文英雄名 / 出装 / 推荐、缺图 / stale 状态和不同英雄明暗原画的 Windows 截图对比；同时验证前景文字对比度、自动档位切换、eco / hidden / reduced-motion 静态路径及真实对局性能。用户视觉确认前不得标 `FIXED / VERIFIED`。

### HB-057 Wallpaper Engine 接入

- 严重度：中高（涉及第三方程序生命周期、安装发现、多屏和性能，错误接入可能造成进程干扰或泄露本地环境）
- 状态：`IN PROGRESS（REPORTED / UNDIAGNOSED）`；用户只提出“客户端接入 Wallpaper Engine”，具体产品语义、支持范围、授权方式与技术路径尚未审计，不得假定为启动壁纸、切换壁纸、嵌入画面或进程控制中的任一种，也不得预写实现。
- 强制边界：必须由用户显式启用并可随时关闭，所有第三方发现 / 生命周期管理只在 Main；Renderer 不得读取或接收安装路径、命令行、进程详情或其他本地环境信息。不得注入、挂钩、篡改或绕过 Wallpaper Engine / Steam，不得复制、打包、上传或分发任何第三方壁纸资产。
- Fail-safe / 验收契约：未安装、未运行、启动失败、关闭、崩溃、版本不兼容、权限拒绝、多屏 / 显示器热插拔和高负载时均须安全降级，不影响 HexBridge 主流程、WeGame / 游戏或用户原有壁纸状态；退出 HexBridge 不得误杀用户自行启动的第三方进程。需先完成可用官方 / 受支持集成面的只读审计，再验证显式开关、所有权、恢复、100%～150% DPI、多屏和 CPU / GPU / FPS / frametime，无证据时保持不可用并说明原因。
...
2026-08-14 v0.1.24 / HB-047 + HB-054 Windows 候选（未 tag / Release）：

- 产品 / 身份：默认关闭、仅本机、`ACCEPTED RISK`；`selecting / launching=champ-select`、`active=gameflow`。唯一 self、跨组唯一、任一 raw team >5 全局 fail closed；4 位队友和 5 位对手分别 all-or-nothing，组间允许 partial，selecting / launching partial 进入 active 后重查。
- 请求 / 样本：固定 Main-only current-summoner + per-PUUID history `endIndex=19`，最多 20 场、少于 12 场不评分；跨被取消旧批次的全局并发 2、2 MiB / Abort / timeout、瞬态失败最多一次 retry。禁用、换局、transport / credential 变化、退出和新批次均取消并清理。
- 隐私 / 明细：每位可用玩家只向主窗口提供 generation 绑定的随机 24 字符 opaqueKey 与脱敏 summary；点击仅 Main sender 且必须同时命中 key + generation + current membership，详情只含英雄 / 胜负 / KDA / 分钟。PUUID、玩家名、participant、gameId、原始载荷、逐局时间戳和路径不进 Renderer / 日志 / 磁盘。对话框具备初始聚焦、Tab / Shift+Tab 焦点陷阱、Escape / 背景关闭和成功 / 失败后的触发头像焦点恢复。
- 本地证据：README 已同步，最终审查无 P0 / P1；clean audit 0、OCR synthetic、真实 4K title-only fixture `137ms`、36 files / 369 passed + 1 skipped、typecheck、lint、source bridge / UI、icon / release-retention / legacy / public-channel verifier 与 diff-check 全过。
- macOS 交叉打包非正式证据：`pack:win` / updater metadata / checksums 通过；EXE `198764829` bytes / SHA-256 `e421c4858e1a8dfd9504ed90ac564de6fc092b2c6c039c03a2a175b37cf6f176`，blockmap `201139` / `c524de5ed3bcf5b355266cbf0b1edbb8e7d6cb425531cb35af80a1e3165b8ba5`，ZIP `274302721` / `be2cea707c2a0fddea62009032144b05a8d2cc0ef1c12972804f08202be35f64`，`latest.yml` `346` / `322d1d642928013dfee638fb88175f45fc91a742012a203f78767a34737bdcd8`，`SHA256SUMS.txt` `182` / `ca48d1720028668312ff3f05c1a5d03f9c6215c785372a85a7334c5038f3c914`，latest SHA-512 `DMpNKFrZmzxN8VnGAQfiYqTSnCrhktVWu7BG7O9EygTjIaxBtYjfkIaO6riaeJGIqpoc6k1hFfwjFLdR8Dh7Og==`。这些是 macOS 交叉构建完整性证据，不是 Windows Actions 正式产物、Windows / WeGame 实机或用户同机验收。
- Windows 候选证据：commit `24a094b4c4dd1b1570b54cdc78d9845e2a0e2d5b` 已 push main；run `31813165279` / job `94808409628` 于 `2026-08-14T15:10:29Z`～`15:16:21Z` success，约 5m52s。clean npm ci / audit 0 / public 0.1.23 / OCR synthetic + 真实 4K `263ms` / Windows 36 files / 370 passed / lint / typecheck / retention / legacy / pack / metadata / packaged UI + bridge / differential / checksums / artifact 全过；EXE `199257694` bytes，packaged UI 验证 augment `960×96` 与校准 `1024×768`。synthetic 0.1.25 差分为 `downloaded=true / differentialDownload=true`，old / new blockmap 各 1、Range 11、传输 `1180063 / 199257694` bytes、`isolatedCache=true`。artifact `9224084775` / `473462227` bytes / digest `sha256:624681ae940d0ba4821c43b9d4bae514b3ba51c0429bcd9522c3852387aae770`。tag-only Release / channel / public 步骤按预期 skip，尚无 v0.1.24 tag / Release，公开 Latest 仍为 v0.1.23。Windows runner 不连接真实 WeGame，不得因此升级 HB 状态。

2026-08-14 v0.1.23 / HB-055 Windows 候选与正式发布：
...
- 本地证据：最终审查无 P0 / P1；audit 0、OCR synthetic、真实 4K fixture 141ms、36 files / 358 passed + 1 skipped、lint、typecheck、source bridge / UI、icon、retention 与 diff-check 全过。
- Windows 候选证据：candidate / HEAD / origin main commit `37b20913c1fdd8be95286bb17bdd44f7fb5a4333`；run `31808228628` / job `94792237502` 于 `2026-08-14T14:10:47Z`～`14:16:45Z` success，约 5m58s。clean npm ci / audit 0 / public channel 0.1.22、OCR synthetic + 真实 4K、Windows 36 files / 359 passed、lint / typecheck / retention / legacy、pack / metadata、packaged UI / bridge、differential、checksums 与 artifact 全过；EXE `HexBridge-0.1.23-x64.exe` 为 `199254230` bytes，packaged UI 验证 augment surface `960x96` 与校准 `1024x768`。artifact `9222211078` / `473456253` bytes / digest `sha256:6147113980021e7f7cd63a1896f73b2419c8ba3a15e8e384ce9dc6cf51552109`。
- 差分 / 发布边界：synthetic 0.1.24 验证 `downloaded=true`、`differentialDownload=true`、old / new blockmap 各 1、installer Range 10、传输 `1165835 / 199254230` bytes 且 `isolatedCache=true`。tag-only Release / channel / public 步骤按预期 skip；尚无 v0.1.23 tag / Release，公开 Latest 仍为 v0.1.22。Windows runner 不连接真实 WeGame，HB-055 继续 `IN PROGRESS / UNVERIFIED`；Node 20 deprecated annotation 为非阻断维护项。
- 正式发布证据：annotated tag object `e53cb90a02c43010c6cfee68758d525a91b108b6` 解引用产品提交 `06ca3f5307133c86bc0f8b5c5cbc8eed76c16d7d`；tag run `31808909709` / job `94794460360` 于 `2026-08-14T14:19:13Z`～`14:26:03Z` success，约 6m50s。Windows 36 files / 359 passed 及全部候选、正式 Release / channel / public 门禁通过；Release ID `370625932` 于 `2026-08-14T14:25:52Z` 发布为 public Latest、non-draft、non-prerelease。public v2 独立核验 `version=0.1.23`、正式 EXE path / size `199254309` 与给定 SHA-512 一致，public packaged check 为 `channelVersion=0.1.23 / updateAvailable=false`。
- 正式差分 / 边界：synthetic 0.1.24 为 `downloaded=true / differentialDownload=true`、old / new blockmap 各 1、Range 11、传输 `1198682 / 199254309` bytes、`isolatedCache=true`。正式 Actions artifact `9222490354` / `473456225` bytes / digest `sha256:3066c1a47a259572ad9fab905347e0170c7b3e7f8edde470498d5eb774cf8352`。v0.1.11～v0.1.23 各保留五资产，legacy 根仍为 0.1.14；Windows runner 仍不等于真实 WeGame 同机，HB-055 保持 `IN PROGRESS / UNVERIFIED`，未签名 / SmartScreen 与 Node 20 annotation 边界不变。
...
- 正式发布证据：annotated tag object `a61033caa796d7b83b920606dc1d8ba1e4da9161` 解引用产品提交 `9fe7e3c80e622e0eb50a5198ac3f753f668049e8`；tag run `31806400237` / job `94786266964` 于 `13:48:00Z`～`13:56:05Z` success，约 8m5s。Windows 36 files / 354 passed + 1 skipped 及 packaged UI / bridge、fake follow、differential、Release、channel、public packaged 全过；Release ID `370607755` 于 `2026-08-14T13:55:56Z` 发布为 public Latest、non-draft、non-prerelease，public v2 独立核验 `0.1.22 / 199254074`。正式发布仍不连接真实 WeGame，HB-048～HB-053 保持 `IN PROGRESS / UNVERIFIED`；HB-054～HB-057 是发布后登记且尚未实现的新目标。

2026-08-14 v0.1.21 / HB-047 最终 dirty 修复候选（未 Windows / 真实 WeGame / 用户同机）：

- 产品决策：用户要求修复“对手战绩无用”，不是删除功能；任何删除方向均已完整撤回。默认关闭、本机实验、受支持队列、Main-only、固定历史端点与隐私边界保持不变，政策 / 自定义分发继续 `ACCEPTED RISK`。
- 身份策略：`selecting / launching` 只认 champ-select 中显式 `VISIBLE` 或显式空字符串，字段缺失 / `HIDDEN` / 未知拒绝；`active` 只认 gameflow 中本方身份与 team 归属恰好唯一、对方恰好 5 个唯一合法 PUUID，并允许可见性字段缺失 / `null` / `VISIBLE`，显式空字符串 / `HIDDEN` / `UNKNOWN` / 未知拒绝。visibility 拒绝优先于 incomplete；active 常省略该字段，因此 v0.1.20 的全阶段严格 `VISIBLE` 会使真实 active 路径不可用。
- 历史 / 错误策略：fulfilled `null` 与结构化 error envelope 明确分类；目标玩家必须在 identity / participant 两侧唯一关联，无 PUUID identity 只在 single participant 的 legacy 形态下 fallback，畸形 / 重复 / 歧义 fail closed。HTTP 500 / 429 仅自动重试一次，404 / schema 错误不重试；并发 2、2 MiB 与 Abort 不变。
- Runtime 策略：首个 active 身份未就绪不会再被 `attemptKey` 永久锁死；只有 source / team incomplete 或 transport Abort 才按 3 / 5 / 10 / 15 / 15 秒有界自动重试，hidden / ambiguous / visibility 拒绝不重试；成功即停止，耗尽后提示手动重试，选人隐藏身份不轮询历史。
- 当前证据：最终审查无 P0 / P1；完整本地 36 files / 350 passed + 1 skipped 及 typecheck / lint / source / OCR / icon / retention / diff-check 全过。test-only commit `2304ee5afc8cdbe89a39f6184d918087426c6f95` 已 push，run `31802621240` / job `94773940537` success 并通过 Windows 36 files / 350 passed + 1 skipped、packaged UI / bridge、differential updater 等候选门禁。runner 未连接真实 WeGame，HB-047 仍为 `IN PROGRESS / UNVERIFIED`，不得写 `FIXED` / `VERIFIED`，政策 / 自定义分发继续 `ACCEPTED RISK`。固定 endpoint、Main-only 与 PUUID 不日志 / 不出 Main / 不进状态 / 不落盘边界不变。

2026-08-14 v0.1.21 / HB-048～HB-053 Windows 候选时间线与正式发布基线：

- HB-048：统一租约为 `active=12h`、`committed=10m`、`weak=15s`；空 ChampSelect、`None` 与 partial observation 不续期，只有正向 `GAME_STARTING` / `gameClient` / 独立游戏进程 / augment 证据才允许相应长租。伴随窗手动关闭按当前 generation 抑制，本局不重开、下一局恢复。
- HB-049：真实 `LeagueClientUx` PID 锚定、Win32 贴边与小幅坐标抖动抑制已进入候选；真实 WeGame 边框、DPI、多屏、移动 / 最小化仍未验。
- HB-050 / HB-052：刷新指纹稳定确认收口为 280ms，最多 4 次有界 full OCR 重试；目标回归确认 3 次 `UNRELIABLE` 后第 4 次 `MATCHED`，且不会出现第 5 次 full OCR。主窗口保留最后 slots，96px compact 仅可靠卡面显示，并在失焦、禁用、刷新、连续两次 absence 或 45 秒时关闭。手动识别后的监测保持低成本有界，不转为持续 full OCR。
- HB-051：compact 同时显示推荐 1 / 2 / 3、rank / 依据和英雄专属 pickRate；pickRate 不排序，且只有 `source + region` 同时命中 allowlist 才展示，否则为 `null / 暂无数据`。
- HB-053：Main-only 缓存 current-summoner PUUID，只在 `active` 阶段与 gameflow team 做唯一匹配恢复本英雄；失败固定 5000ms 退避。目标回归确认首次 `null` 后 4999ms 不重试、5000ms 成功，且 snapshot / logger 无 PUUID；PUUID 不得进入 Renderer、IPC 业务状态或磁盘。
- 本地 / Windows 证据：产品候选本地 36 files / 350 passed + 1 skipped、typecheck、lint、source bridge / UI、OCR synthetic、真实 4K fixture（141ms）、retention、icon 与 `git diff --check` 全过，最终审查无 P0 / P1。首轮 `31799661557` 因 fake BrowserWindow 缺 `getNativeWindowHandle` 失败，第二轮 `31802292958` 因 dismissal 测试未模拟 observer gate 失败；test-only 修复提交 `2304ee5afc8cdbe89a39f6184d918087426c6f95` 已 push main。第三次 workflow_dispatch run `31802621240` / job `94773940537` 于 `12:58:32Z`～`13:03:52Z` success，约 5m20s；clean npm ci / audit / public 0.1.20、OCR synthetic + 真实 fixture、Windows 36 files / 350 passed + 1 skipped、lint / typecheck / retention / legacy、pack / metadata、packaged UI / bridge、differential updater、checksums、artifact 全过。artifact `9220014016` / `473454828` bytes / digest `sha256:3af41d279d19419befadaf9aee3f36100268bbe6d4316efa786ccfca31452003`；tag-only Release / channel / public 步骤按预期 skip。真实 WeGame 与用户同机均未完成，HB-048～HB-053 全部继续 `IN PROGRESS / UNVERIFIED`。
- 正式发布证据：annotated tag object `719f10ed8af757a9c463cbaeccab51ea77945307` 解引用产品提交 `24e6d708671fbd1807ab1c973e01e6b40de43245`；tag run `31803207522` / job `94775818465` 于 `13:06:19Z`～`13:12:06Z` success，约 5m47s。Windows 36 files / 350 passed + 1 skipped 与全部 packaged / differential / Release / channel / public smoke 通过；Release ID `370582774` 于 `2026-08-14T13:11:56Z` 发布为 public Latest、non-draft、non-prerelease，public v2 channel 独立核验 `0.1.21 / 199253519`。发布不连接真实 WeGame，HB-047～HB-053 状态不升级。

2026-08-14 v0.1.20 / HB-047 本地候选阶段（随后已进入 `cd8992a` Windows 候选，仍未 tag / Release）：
...
- 最后阶段化身份修复后的完整本地证据为 36 test files / 306 passed + 1 skipped、typecheck、lint、source bridge、source UI 与 `git diff --check` 全过；独立终审无已知 P0 / P1并批准进入 Windows 候选。后续 Windows 候选证据见下一段；真实 WeGame / LCU endpoint、真实 2400 / 自定义 3270 身份载荷、隐私泄漏检查或性能实测仍不存在，因此实现仅为 `FIXED / UNVERIFIED`，政策 / 自定义分发仍为 `ACCEPTED RISK`。

2026-08-14 `v0.1.20` / HB-047 Windows workflow_dispatch 候选（已 push，未 tag / Release）：

- 产品 / 候选 commit `cd8992ae6d8c34bd4b5d8ce68e8885f41607ac0e` 已 push main。workflow_dispatch run `31779716617` / job `94702618479` 于 `2026-08-14T07:23:09Z`～`07:28:33Z` success，约 5m24s。
- clean `npm ci`、Electron hydrate、audit high 0、public v0.1.19 channel、OCR synthetic + real fixture、Windows 36 test files / 306 passed + 1 skipped、lint、typecheck、retention、legacy、pack / updater metadata、packaged UI、packaged bridge（含 native window follow synthetic）、differential update、checksums 与 artifact 全过；tag-only Release / channel 步骤按预期 skip。
- Actions artifact ID `9211328981`，size `473444447` bytes，digest `93d17c3eeaf5aaae6ca27f9b52b626de38574a71121d68e6e1317671886aa01c`。该 artifact 只是候选产物，不是正式 Release 资产；公开 Latest 仍为 v0.1.19，v0.1.20 尚无 tag / Release。
- 本次 CI 覆盖 packaged 边界和合成原生窗口跟随，但不连接真实 WeGame / LCU；HB-047、HB-044 / HB-046 与 HB-045 总体继续 `FIXED / UNVERIFIED`，只有 HB-045 synthetic native-window follow 的既有窄范围可记 `VERIFIED`。真实 2400 / 自定义 3270 身份载荷、对手历史 endpoint、隐私泄漏、DPI / 多屏、点击穿透与游戏性能状态均不得升级。

2026-08-14 `v0.1.20` 正式 Release 基线：

- annotated tag `v0.1.20` 解引用到产品 / 标签提交 `4aecc9199f1d95fd5dd88fce1a29645180d2a368`。tag run `31780353884` / job `94704573925` 于 `07:33:05Z`～`07:38:39Z` success，约 5m34s；Windows 36 files / 306 passed + 1 skipped，以及 clean dependencies、OCR、lint / typecheck、retention / legacy、pack / metadata、packaged UI / bridge（含 native-window-follow synthetic）、differential update、checksums、Release、channel 和 public packaged check 全通过。
- Release [v0.1.20](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.20) ID `370421727`，publishedAt `2026-08-14T07:38:30Z`，发布时为 public、Latest、non-draft、non-prerelease，当前已由 v0.1.21 取代 Latest。正式 Actions artifact ID `9211571523`，size `473444531` bytes，digest `0a85291404c942550b6687eb51e8bc95def2026cc98bf2c6b23bf19c507832d7`。
- 正式五资产：EXE `199249536` bytes / SHA-256 `3eea47705fefe7d9c7a4f3edf0b8b8ebfd45d265b0635d35420fdcff9e5cc017`；blockmap `201342` / `6ccd7f35a3fd3e7dc58ea387700bc37f2e3a18c1a0322fafc9730e535872a9a4`；ZIP `274405587` / `9ae32030aabe8791de0a5acb1a32f9d6fbe29f6bf659c49fad5d7d3e24f955d0`；`latest.yml` `346` / `b48a6cb287356c8cd95d5294cbce243a04bdb65dd516d1a52e6580fa10027b46`；`SHA256SUMS.txt` `182` / `75fa233d50b115eaeae06f903b6aa2e438398b84c8acb1cafe38a439be34b2f8`。
- 在 v0.1.20 发布时 stable 路径 `update-channel/v2/latest.yml` 的 version `0.1.20`、path、size `199249536` 与 SHA-512 均和当时正式 EXE 一致，channel branch commit 为 `60252b27…`；当前 stable 已由 v0.1.23 取代。legacy 根 `latest.yml` 固定保留 0.1.14 是既定兼容边界，不是更新故障。v0.1.11～v0.1.23 Releases / assets 全部保留。
- 正式发布不改变实机状态：HB-044 / HB-046 / HB-047 和 HB-045 总体继续 `FIXED / UNVERIFIED`，只有 HB-045 synthetic native-window follow 既有窄范围为 `VERIFIED`；HB-047 政策 / 自定义分发继续 `ACCEPTED RISK`。真实 WeGame、2400 / 自定义 3270 身份 / endpoint / 隐私、DPI / 多屏、点击穿透和性能仍待验。Node 20 action annotation 非阻断，无商业代码签名 / SmartScreen 边界不变。
...
- 产品提交 `5bcad430d738471a72cfe4990c5474ffc4d7b402` 后，首次 workflow_dispatch run `31775889265` / job `94691136343` 在 packaged UI 阶段失败。失败原因是旧烟测错误要求 Windows idle / 未确认新版状态必须显示 update button；产品契约与 HB-041 一致，只有 Main 已确认存在高于当前版本的可用更新时才显示按钮，因此不是产品回归。
- 修复范围严格限于 `scripts/electron-ui-smoke.mjs`，让 smoke 按真实更新契约验证 idle 状态隐藏按钮；commit `97cc10665b70883e643ab0ab5c8b32fc02df1a18` 已 push main。该修复的 source UI、lint、diff-check、Node 语法检查通过，最终独立审查无已知 P0 / P1。
...
- tag-only Release / channel 步骤按预期 skip；公开 Latest 仍为 v0.1.19，v0.1.20 尚未 tag / Release。Windows runner 不替代真实 WeGame `LeagueClientUx`、多屏 / 混合 DPI / 最小化、点击穿透、游戏 CPU / GPU / FPS / frametime或用户同机验收；HB-044 / HB-046 保持 `FIXED / UNVERIFIED`。该 run 位于后续 HB-047 实现之前，不能为 HB-047 提供 Windows 证据；其当时的纯 `ACCEPTED RISK` 状态已由“实现 `FIXED / UNVERIFIED` + 分发政策 `ACCEPTED RISK`”取代。
...
- 产品 HEAD / origin main 为 `748eaab42cd11644b26c75f044335d5ad2ce5da3`。首次候选 run `31770633134` 因设置页校准 smoke 错读已不存在的 `connection-stage` 失败，修复为永存 stage；第二次 run `31770956164` 在慢 runner 上两次 1.8 秒 `Get-Process` 不可用，随后把产品 1.8 秒预算与 CI 8 秒同生产脚本验证拆开，并增加从入口起 24 秒绝对 deadline 与外层 30 秒清理余量。两次均为候选门禁问题，不得写成产品已发布失败。
- 第三次 workflow_dispatch run `31771597588` / job `94678512664` 于 2026-08-14T04:59:54Z～05:04:17Z success，约 4m23s：clean `npm ci`、audit 0、public 0.1.18、OCR synthetic、真实 4K fixture 202ms（由心及物 / 冰寒 / 虹吸）、Windows 31 files / 279 tests、lint、typecheck、retention / legacy channel、pack、metadata、packaged UI / calibration、packaged bridge、synthetic differential updater、checksums 与五文件 artifact 全通过。
- packaged UI 证据：`titleHasVersion=false`、`updateNavigation=false`；校准截图 1024×768；恢复状态 `hidden=false / focused=false / paused=true`。候选 EXE 为 199,239,210 bytes。
- synthetic updater 从 previous 0.1.18 到 0.1.20：`differential=true`、metadata 1、old / new blockmap 各1、Range 11、redirect 3、传输 1,117,915 / 199,239,210 bytes、`isolatedCache=true`。五文件 Actions artifact ID `9208397307`。
- tag-only 发布步骤按预期 skip；尚未 tag / Release，公开 Latest 仍为 v0.1.18。HB-041～043 均保持 `IN PROGRESS`，Windows runner 不替代用户视觉或真实 4K 游戏性能，不能写 `FIXED` / `VERIFIED`。

2026-08-14 `v0.1.19` 正式 Release 基线：

- annotated tag object `a44b758fcbe05ec3744b4c6afa57b1622929c9ef` 指向产品 / 候选记忆 commit `0b30064d399c69d7edc8f082d0ef27b40bc8c5d3`。正式 tag run `31772021173` / job `94679735360` 于 2026-08-14T05:07:41Z～05:13:38Z success，约 5m57s；31 files / 279 tests、真实 4K fixture 275ms、lint、typecheck、retention、packaged UI / calibration / bridge、synthetic differential 0.1.20、checksums、Release / channel / public packaged check 全通过。
- synthetic updater 从 previous 0.1.18 到 0.1.20：`differential=true`、Range 11、传输 1,117,913 / 199,239,208 bytes；public packaged 返回 `{ channelVersion: 0.1.19, updateAvailable: false }`。public channel 独立回读 `0.1.19 / 199,239,208 bytes`。
- Release [v0.1.19](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.19) ID `370366810`，publishedAt `2026-08-14T05:13:29Z`，为 Latest、non-draft、non-prerelease。
- 正式五项资产：
  - EXE：199,239,208 bytes，SHA-256 `785c4c9db130ec3d8045a47d3145c18276387741276a5a468ce43b464b751e74`。
  - blockmap：201,181 bytes，SHA-256 `9d83cc38430ae8cb3d59b846d80347f1e04a004982f5c3b596fd44739f278106`。
  - ZIP：274,392,932 bytes，SHA-256 `0a2153f5fd5308087cd3169987c19bc20c8a711d599cba890215df25f9ce22e0`。
  - `latest.yml`：346 bytes，SHA-256 `13b9e810136ce334a90a258462641511c97619f825514103b730c6b618c9b5c5`。
  - `SHA256SUMS.txt`：182 bytes，SHA-256 `1d1b350519b2595c2c274547ade4cbc5816414340c5fa63b4530f30ddbab0ba2`；清单内容与正式 EXE / ZIP SHA-256 一致。
- v0.1.18 Release 的五项资产仍完整保留，tags 保留。发布成功不替代真实 Key、用户视觉、真实 4K 游戏 CPU / GPU / FPS / frametime或 installed 更新验收；HB-041～043 保持 `IN PROGRESS`。当前未商业签名，SmartScreen 边界不变。
...
- **最新 OCR 交互实机（HB-025 / HB-029 / HB-046）**：v0.1.13 报告用户同机确认手动按钮可以识别，但游戏前台配置快捷键无反应；识别后出现的黑屏顶部浮窗也被用户明确否定。v0.1.14 因此让 button / hotkey / tray 共用 Main 触发路径，移除当时的 augment BrowserWindow / route / component，并使结果只进入实时助手且同步不 show / focus。当前 HB-046 又按用户新目标加入严格 96px、透明、点击穿透、不可聚焦的小型卡下推荐窗；这不撤销对旧全屏 / 顶部黑窗的禁止。当前 source 门禁仍不能替代 Windows 游戏前台快捷键、小窗、刷新换代或用户同机复测。
...
- HB-029～HB-031 当前均为 `IN PROGRESS`。v0.1.14 删除了产生黑屏体验的 augment BrowserWindow / `#augment` route / `AugmentOverlay.vue`，并把 OCR 结果收口到实时助手；当前 HB-046 仅新增严格 96px、透明、点击穿透、不可聚焦的卡下小窗窄例外，旧全屏 / 顶部黑窗仍永久禁止。英雄专属 `pickRate` 继续按数值与来源 / 区域 allowlist 清洗且不改变 rank 排序，详情缓存与 Tier 原始文本契约不变。既有正式 Windows tag run 与当前 source 门禁都不能替代游戏前台快捷键、真实 4K 小窗 / 数据 / 视觉和用户同机验证，三项不得写 `VERIFIED`。
...
- HB-033～HB-038 当前均为 `IN PROGRESS`。v0.1.15～v0.1.23 的 Windows 差分烟测均在受控 cache 前置条件下验证 Range / blockmap 与候选 / 正式门禁；但用户 v0.1.14→v0.1.15 仍看到 / 可能下载约 200 MB，尚无其 installed 客户端网络字节证据区分 metadata 展示与 full fallback。必须由 installed 客户端对 v0.1.23 或后续正式版复验；Windows 图标、合成器隐藏截图恢复，以及报告用户同机多轮三卡 / 中文依据 / toast 也仍未验。公开 Latest 为 v0.1.23。
...
- HB-047 当前为 `IN PROGRESS / UNVERIFIED`，政策 / 自定义分发为 `ACCEPTED RISK`：用户要求修复而非移除战绩功能，删除方向已撤回。v0.1.24 Windows 候选把既有阶段 visibility / 唯一 join / 瞬态一次重试扩展到 4 位队友 + 5 位对手：唯一 self、跨组唯一、raw team >5 全局拒绝；两组各自 all-or-nothing、组间可 partial；固定 `endIndex=19`、最多 20 场 / 12 样本、跨批次全局并发 2，并用 generation-bound 24 字符 opaqueKey 提供 Main-only 脱敏明细。终审无 P0 / P1，本地 36 files / 369 passed + 1 skipped 与 Windows 36 files / 370 passed 及全部候选门禁通过，但尚无 tag / Release 或真实 WeGame 证据，不得标 `FIXED / VERIFIED`。
...
- HB-054 当前为 `IN PROGRESS / UNVERIFIED`：v0.1.24 Windows 候选已实现队友 / 对手分组、组间 partial、20 场 / 12 样本、generation 绑定 opaqueKey 和点击头像脱敏明细；Renderer 详情只含英雄 / 胜负 / KDA / 分钟，并有对话框焦点陷阱 / Escape / 失败恢复。README、终审、本地 369+1 与 Windows 370 tests 及完整候选门禁通过，但尚未 tag / Release；真实 WeGame 身份 / history、隐私、清理、性能和用户同机均未验证，政策继续 `ACCEPTED RISK`。
- HB-055 当前为 `IN PROGRESS / UNVERIFIED`：v0.1.23 正式版已修复默认 `autoOcr=false` 下切出前台会清 overlay / deadline、进而让 observer 永久熄灭的自动化根因；切出只 pause / hide，切回 show + cheap probe 且不跑 full OCR，并有独立 45 秒零截图 expiry 与完整撤销条件。正式 Windows tag 已通过 36 files / 359 passed 与完整 packaged / differential / Release / channel / public 门禁，但仍需报告用户同机覆盖切出 / 切回、到期、absence、refresh 和开关切换；不得与 HB-052 的“卡面消失后小条常驻”混淆，也不得写 `FIXED / VERIFIED`。
...
- v0.1.23 发布状态：annotated tag object `e53cb90a02c43010c6cfee68758d525a91b108b6` 解引用到产品 / 标签 commit `06ca3f5307133c86bc0f8b5c5cbc8eed76c16d7d`；正式 run `31808909709` / job `94794460360` 于 `2026-08-14T14:19:13Z`～`14:26:03Z` success，约 6m50s。Release [v0.1.23](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.23) ID `370625932`，publishedAt `2026-08-14T14:25:52Z`，为 public、Latest、non-draft、non-prerelease；Windows 36 files / 359 passed 及全部候选、正式 Release / channel / public 门禁通过。
- v0.1.23 正式资产：EXE `199254309` bytes / SHA-256 `d2d5cbe32602e57c76ea1b727fd79ad72af8c026fe76e452fd14e5b2f3b48759`；blockmap `201331` / `552aca51aa82716c3170a597eb2a345393d0bb1e8614ad00006494145fbf42b5`；ZIP `274411692` / `f159a2751bf73f0102b15a7d661b71e1eeec3220b6f7b648db061774536568b5`；`latest.yml` `346` / `3dff30e434bdb1a824232b088afd9ec180c3b62be7cadf779d26722bc7b857bb`；`SHA256SUMS.txt` `182` / `3aeac2dfbb8effdfe0d664a2ff292e5a61210e00d478a0ef6b9bc4bba6420f93`。`SHA256SUMS.txt` 内 EXE / ZIP 与对应 assets 一致；Actions artifact `9222490354` 为 `473456225` bytes，digest `sha256:3066c1a47a259572ad9fab905347e0170c7b3e7f8edde470498d5eb774cf8352`。
- v0.1.22 发布状态：annotated tag object `a61033caa796d7b83b920606dc1d8ba1e4da9161` 解引用到产品 / 标签 commit `9fe7e3c80e622e0eb50a5198ac3f753f668049e8`；正式 run `31806400237` / job `94786266964` 于 `13:48:00Z`～`13:56:05Z` success，约 8m5s。Release [v0.1.22](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.22) ID `370607755`，publishedAt `2026-08-14T13:55:56Z`，发布时为 public、Latest、non-draft、non-prerelease，当前已由 v0.1.23 取代 Latest；Windows 36 files / 354 passed + 1 skipped 及 packaged UI / bridge、fake follow、differential、Release、channel、public packaged smoke 全过。
- v0.1.22 正式资产：EXE `199254074` bytes / SHA-256 `360611b9a13616c1d9271e182b8297cfdde4e7de9258a96500a5ea3417a794f5`；blockmap `201462` / `28a58f8d162b57247936067ee3ec5b91d779cb25c1b589aea8b68247b586125f`；ZIP `274411455` / `7c50723da8da96b09c0e1c848e46cebd8766ddb6f5ddbf0d6e20990e34fb55a1`；`latest.yml` `346` / `039f6ec9d440605f4853658257d370d2b98e62c4e2a8d29990da72068bb98e1e`；`SHA256SUMS.txt` `182` / `f9b2da6f8f190eed4e99737ca990496d18fff3d8e01bad3756afd0dfab9a0b74`。Actions artifact `9221561761` 为 `473455761` bytes，digest `sha256:366547c64b09c44cfecf02ae318d92aba8f24968256ce1fc201574ade2cb9797`。
- v0.1.21 发布状态：annotated tag object `719f10ed8af757a9c463cbaeccab51ea77945307` 解引用到产品 / 标签 commit `24e6d708671fbd1807ab1c973e01e6b40de43245`；正式 run `31803207522` / job `94775818465` 于 `13:06:19Z`～`13:12:06Z` success，约 5m47s。Release [v0.1.21](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.21) ID `370582774`，publishedAt `2026-08-14T13:11:56Z`，发布时为 public、Latest、non-draft、non-prerelease，当前已由 v0.1.22 取代 Latest；Windows 36 files / 350 passed + 1 skipped 及全部 packaged / differential / Release / channel / public smoke 通过。
- v0.1.21 正式资产：EXE `199253519` bytes / SHA-256 `36e63e6a079dcc51fe898283733379e4d027f049b96e25e1885db30558ca43db`；blockmap `201352` / `e0c4b0caa92b2980711c67dda29a076a32287244a4b0448518d02cfa169f0fec`；ZIP `274410840` / `6921e8029949b95993e8b149fdec77ce347454a68e57c28734e7f601c05385c2`；`latest.yml` `346` / `38156f93bba386c18d4e94447defdd0335e8664606488753cb5f8d235e44357c`；`SHA256SUMS.txt` `182` / `3203112ddda15e5087320c3380c9db7bdc29305168c78114cbb3e83803c3d447`。Actions artifact `9220233595` 为 `473454730` bytes，digest `sha256:218e1d8b2102cfbfdbc3713a28edf660574d09301c8392b5dfeb2fd0d943d092`。
- v0.1.20 发布状态：annotated tag `v0.1.20` 解引用到产品 / 标签 commit `4aecc9199f1d95fd5dd88fce1a29645180d2a368`；正式 run `31780353884` / job `94704573925` success，约 5m34s。Release [v0.1.20](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.20) ID `370421727`，publishedAt `2026-08-14T07:38:30Z`，发布时为 public、Latest、non-draft、non-prerelease，当前已由 v0.1.21 取代 Latest；完整 packaged / differential / Release / channel / public check 门禁通过。
- v0.1.20 正式资产：EXE `199249536` bytes / SHA-256 `3eea47705fefe7d9c7a4f3edf0b8b8ebfd45d265b0635d35420fdcff9e5cc017`；blockmap `201342` / `6ccd7f35a3fd3e7dc58ea387700bc37f2e3a18c1a0322fafc9730e535872a9a4`；ZIP `274405587` / `9ae32030aabe8791de0a5acb1a32f9d6fbe29f6bf659c49fad5d7d3e24f955d0`；`latest.yml` `346` / `b48a6cb287356c8cd95d5294cbce243a04bdb65dd516d1a52e6580fa10027b46`；`SHA256SUMS.txt` `182` / `75fa233d50b115eaeae06f903b6aa2e438398b84c8acb1cafe38a439be34b2f8`。Actions artifact `9211571523` 为 `473444531` bytes，digest `0a85291404c942550b6687eb51e8bc95def2026cc98bf2c6b23bf19c507832d7`。
- 当前更新通道 / Git：stable 元数据使用 `update-channel/v2/latest.yml`；public v2 channel 已独立核验 version `0.1.23`、path 指向正式 EXE、size `199254309`、SHA-512 `5ECDzDf8PksDGZRF+gtNJarBcCPEe//LJT5bQx/Do43IIbrrqCrcR0uKxrzJk1Sonj10VIEW1m/RO/KOOzUbGQ==`，public packaged check 返回 `channelVersion=0.1.23 / updateAvailable=false`。legacy 根 `latest.yml` 保留 0.1.14 是兼容冻结，不是发布或更新故障。main 在后续记忆提交后可领先 v0.1.23 tag，但 Release 产品源码固定为 tag 解引用的 `06ca3f5307133c86bc0f8b5c5cbc8eed76c16d7d`。v0.1.11～v0.1.23 正式 Releases / assets / tags 均保留，不得移动、改写或删除。
...
- `v0.1.0` 曾含 HB-012 preload 阻断缺陷，其 Release 已删除，不能作为功能可用基线。当前公开推荐版本为 `v0.1.23`；但 tagged packaged smoke 通过仍不代表真实 WeGame / LCU / 对局 OCR 实机验收。
- `v0.1.23` 是当前公开推荐版本；v0.1.11～v0.1.23 Releases 的远端保留契约持续生效。HB-020 与 HB-022 仍处于 `IN PROGRESS`；用户同机已覆盖连续两局的本局英雄保留、终局清理与第二局换代，但 OCR / 推荐与性能仍未闭环。HB-014 仅在首帧黑屏 / packaged Windows 受控截图与 Esc 恢复窄范围 `VERIFIED`；HB-013、HB-015～HB-018 及 HB-014 的实机剩余范围仍为 `FIXED / UNVERIFIED`。HB-019 的下载 / 差分与安装分流只有自动化窄证据，真实 installed 客户端更新 / 安装链仍未验证。
...
- GitHub Actions 仍有 Node 20 deprecated annotation。当前 workflow 中第三方 actions 固定到完整 commit SHA，runner 已临时强制它们使用 Node 24；`v0.1.23` 正式 run `31808909709` 已成功，因此该 annotation 仍为非阻断维护项。
...
- 2026-08-14 | v0.1.20 / HB-047 本地个人实验候选阶段 | 实现 FIXED / UNVERIFIED；政策 / 自定义分发 ACCEPTED RISK | 默认关闭、Main-only；受支持海克斯队列 `{2400, 3270}` 含用户实际使用的自定义3270；selecting/launching=champ-select、active=gameflow，恰好5个唯一VISIBLE身份后才固定GET current-summoner与逐PUUID `begIndex=0&endIndex=9` history；2MiB、并发2、Abort；最近可用LoL样本少于8局不评分/不分档 | 最后阶段化修复后36 files/306 passed+1 skipped、typecheck、lint、source bridge、source UI、diff-check全过；独立终审无P0/P1并批准Windows候选 | PUUID/raw/history不出Main、不日志/落盘/上传，非Main state剥离，update-settings/retry仅Main；该时点尚无Windows Actions或真实WeGame证据，不得VERIFIED | 随后已进入下方`cd8992ae` Windows候选；公开Latest仍v0.1.19
- 2026-08-14 | v0.1.20 / HB-047 Windows 候选 | workflow_dispatch SUCCESS；HB-047保持FIXED / UNVERIFIED，政策 / 自定义分发保持ACCEPTED RISK | 产品/候选commit `cd8992ae6d8c34bd4b5d8ce68e8885f41607ac0e`已push main；默认关闭、Main-only、受支持队列、阶段化5个VISIBLE身份、有界history与隐私契约进入Windows候选 | run31779716617/job94702618479于07:23:09Z～07:28:33Z约5m24s success；clean npm ci/hydrate/audit high0/public0.1.19/OCR synthetic+real/Windows36 files306 pass+1 skip/lint/typecheck/retention/legacy/pack+metadata/packaged UI+bridge含native follow synthetic/differential update/checksums/artifact全过；artifact9211328981/473444447 bytes/digest `93d17c3eeaf5aaae6ca27f9b52b626de38574a71121d68e6e1317671886aa01c` | CI不连接真实WeGame；2400/自定义3270身份载荷、LCU history端点、隐私、多屏/DPI/点击穿透/性能均不得VERIFIED | tag-only按预期skip；v0.1.20未tag/Release，公开Latest仍v0.1.19
- 2026-08-14 | v0.1.20 / 正式 Release | Release SUCCESS；HB-044/046/047及HB-045总体状态不升级，HB-047政策ACCEPTED RISK不变 | annotated tag `v0.1.20`解引用产品/标签commit `4aecc9199f1d95fd5dd88fce1a29645180d2a368`；默认关闭对手实验、窗口跟随、小型推荐窗与滚动/动效修复正式发布 | run31780353884/job94704573925于07:33:05Z～07:38:39Z约5m34s success；Windows36 files306 pass+1 skip及packaged/differential/Release/channel/public check全过；五资产SHA-256与artifact9211571523/473444531 bytes/digest `0a85291404c942550b6687eb51e8bc95def2026cc98bf2c6b23bf19c507832d7`已记录；v2 channel0.1.20/199249536/SHA512一致 | 真实WeGame 2400/自定义3270身份/endpoint/隐私、多屏/DPI/点击穿透/性能仍UNVERIFIED；仅既有native follow synthetic窄范围VERIFIED；Node20 annotation非阻断、无商业签名/SmartScreen边界不变 | Release ID370421727，于07:38:30Z公开Latest/non-draft/non-prerelease；v0.1.11～v0.1.20资产保留，legacy root0.1.14为兼容冻结
- 2026-08-14 | HB-048～HB-051 / v0.1.20 用户实机回归 | IN PROGRESS（REPORTED / UNDIAGNOSED） | 登记自定义3270退出后hero/detail/overlay/伴随窗残留且伴随窗不可关闭；真实LeagueClientUx跟随不贴合并乱移动；刷新三卡后OCR/推荐更新慢；三卡须显示英雄专属真实pickRate而非rank | 仅冻结现象与验收契约；未运行测试、构建或新增实现，不预写根因 | 需验证退出房间有界清理、本局手动关闭不重开且下一局恢复；真实Windows边框/DPI/移动/最小化稳定贴合；刷新后有界换代；每卡pickRate百分比或null暂无数据，rank仅作推荐顺序 | 仅更新项目记忆；不得记录token/PUUID/path/raw session，无commit/tag/Release变更
- 2026-08-14 | HB-052 / 选择卡牌后96px推荐条不消失 | IN PROGRESS（REPORTED / UNDIAGNOSED） | 登记主窗口可靠结果与游戏内小条生命周期必须分离：小条只在三卡界面实际存在时显示，选择完成后不得常驻 | 仅冻结问题和显隐/性能契约，未运行测试、构建或实现，不预写根因 | 连续可靠absence后隐藏；刷新先撤旧条、3/3新结果后再显示；autoOcr关闭时手动成功后只允许有界低成本存在性监测，不得持续full OCR或增加游戏性能峰值 | 仅更新项目记忆；无commit/tag/Release变更
- 2026-08-14 | HB-048～HB-053 / v0.1.21 Windows 首跑与 dirty smoke 修复 | 全部 `IN PROGRESS / UNVERIFIED`，状态不升级 | 既有候选契约不变；Windows lifecycle fake BrowserWindow 缺少 `getNativeWindowHandle`，dirty 修复为 native handle 获取增加方法 / Buffer 守卫与 catch 后 fail closed | candidate commit `c12b3a4` 已push main；run31799661557/job94764404770在Windows npm test失败，失败前prechecks/audit/public/OCR通过且未build/release；dirty修复目标2 files/14 tests、typecheck/lint/diff通过 | 修复审查无P0/P1并批准Windows retry，P2全零Buffer为非阻断边界；仍待commit/push/retry，真实WeGame/用户同机未完成，HB状态不升级 | 尚无v0.1.21 tag/Release；公开Latest仍v0.1.20
- 2026-08-14 | HB-047 / v0.1.21 最终 dirty 修复候选 | `IN PROGRESS / UNVERIFIED`；政策/自定义分发 `ACCEPTED RISK` | 功能不删除；selecting/launching仅champ-select，active仅gameflow；分阶段visibility、本方/team恰好唯一、5个唯一对手且visibility拒绝优先；fulfilled null/error envelope分类；history两侧唯一join、无PUUID identity+single participant legacy fallback、畸形/重复fail closed；500/429一次重试而404/schema不重试；Runtime仅source/team incomplete或transport Abort按3/5/10/15/15有界重试，hidden/ambiguous不重试，成功停止/耗尽手动 | 最终审查无P0/P1；完整36 files/350 passed+1 skipped、typecheck/lint/source bridge/UI、OCR synthetic、真实4K 141ms、icon/retention/diff全过 | 固定history endpoint、并发2、2MiB、Abort、Main-only、PUUID不出Main/日志/状态/磁盘边界不变；nativeHandle fail-close及Windows retry待办继续保留 | Windows/真实WeGame/用户同机未完成，HB-047不得FIXED/VERIFIED；公开Latest仍v0.1.20
...
- 2026-08-14 | v0.1.23 / HB-055 Windows 候选 | workflow_dispatch SUCCESS；HB-055 保持 `IN PROGRESS / UNVERIFIED` | candidate/HEAD/origin main `37b20913c1fdd8be95286bb17bdd44f7fb5a4333`；只包含既有HB-055 pause/hide、cheap probe、45秒expiry与撤销链修复，HB-052不变 | run31808228628/job94792237502于14:10:47Z～14:16:45Z success约5m58s；clean npm ci/audit0/public0.1.22/OCR synthetic+真实4K/Windows36 files359 passed/lint/typecheck/retention/legacy/pack/metadata/packaged UI+bridge/differential/checksums/artifact全过；EXE199254230，augment960x96，校准1024x768；差分synthetic0.1.24 downloaded+differential=true、blockmap各1、Range10、传输1165835/199254230、isolated cache；artifact9222211078/473456253/digest `sha256:6147113980021e7f7cd63a1896f73b2419c8ba3a15e8e384ce9dc6cf51552109` | Windows runner不等于真实WeGame同机，仍需切出/切回、expiry与撤销链实测；Node20 annotation非阻断 | tag-only release/channel/public按预期skip；尚无v0.1.23 tag/Release，公开Latest仍v0.1.22
- 2026-08-14 | v0.1.23 / 正式 Release | Release SUCCESS；HB-055 保持 `IN PROGRESS / UNVERIFIED`，HB-052边界不变 | annotated tag object `e53cb90a02c43010c6cfee68758d525a91b108b6` 解引用产品commit `06ca3f5307133c86bc0f8b5c5cbc8eed76c16d7d`；run31808909709/job94794460360于14:19:13Z～14:26:03Z success约6m50s | Windows36 files/359 passed及候选+正式Release/channel/public全门禁通过；五资产/SHA256SUMS一致性与artifact9222490354/473456225/digest `sha256:3066c1a47a259572ad9fab905347e0170c7b3e7f8edde470498d5eb774cf8352`已记录；public v2核验0.1.23/199254309/SHA512且packaged updateAvailable=false；正式差分synthetic0.1.24 blockmap各1、Range11、1198682/199254309、isolated cache | Windows runner不等于真实WeGame同机，HB-055仍需用户切出/切回、expiry与撤销链实测；未签名/SmartScreen及Node20 annotation边界不变 | Release ID370625932于2026-08-14T14:25:52Z公开Latest/non-draft/non-prerelease；v0.1.11～v0.1.23各保留五资产，legacy根仍0.1.14
- 2026-08-14 | v0.1.24 / HB-047 + HB-054 Windows 候选 | workflow_dispatch SUCCESS；HB-047 / HB-054 保持 `IN PROGRESS / UNVERIFIED`，政策 / 自定义分发 `ACCEPTED RISK` | candidate `24a094b4c4dd1b1570b54cdc78d9845e2a0e2d5b` 已push main；队友4+对手5、组间partial、20场/12样本、generation-bound opaqueKey脱敏明细和焦点管理进入Windows候选 | run31813165279/job94808409628于15:10:29Z～15:16:21Z success约5m52s；Windows36 files/370 passed，OCR真实4K263ms，pack/metadata/packaged UI+bridge/differential/checksums/artifact全过；EXE199257694，synthetic0.1.25 Range11传输1180063/199257694；artifact9224084775/473462227/digest `sha256:624681ae940d0ba4821c43b9d4bae514b3ba51c0429bcd9522c3852387aae770` | Windows runner不连真实WeGame，真实身份/history、用户同机、隐私/性能仍未验，不得FIXED/VERIFIED | tag-only Release/channel/public按预期skip；尚无v0.1.24 tag/Release，公开Latest仍v0.1.23；Node20 annotation为非阻断维护项
