...
> 最后更新：2026-08-13
> 当前基线：公开最新正式 Release 为 `v0.1.11`，annotated tag 固定指向产品提交 `20debe3483d8747008de240a9c9a1adcb2304c08`，tag object 为 `4141f3ed…`；`v0.1.12` 候选尚未 tag / Release。此前误把“删除旧版”理解为删除 GitHub 历史 Release，导致 `v0.1.0`～`v0.1.10` 中已发布版本的远端 Release / assets 被删除且不可恢复；其 Git tags 与源码历史仍全部保留；从 v0.1.11 起必须保留以后全部正式 GitHub Releases / assets / tags，只清理本地 `release/`。`v0.1.12` 候选包含 HB-020 / HB-022 交接保留、HB-024 整卡校准到标题 ROI / 保存前 3/3 OCR、HB-025 原子与可配置热键、HB-026 默认关闭自动 OCR + 2 秒低分辨率门控、HB-027 普通未启动提示，以及独立更新页、英雄榜 / UI / icon、远端 Release 保留与本地 cleanup 门禁。packaged UI 焦点语义修正复审无 P0 / P1，commit `0f7b8b9` 已 push；Windows workflow_dispatch run `31682463869` / job `94390815147` 基于完整 commit `0f7b8b93…` 成功，用时约 5m7s。clean npm ci / audit、public channel `0.1.11`、OCR 合成 + 真实 4K、22 test files / 179 passed + 1 Windows skip、lint、typecheck、retention、pack、metadata、packaged UI / bridge、synthetic `0.1.13` updater、checksums 和 artifact 全通过；tag-only 发布步骤按预期 skip。HB-020 / HB-022 及 HB-023～HB-027 一律保持 `IN PROGRESS`：Windows runner 不等于用户同机真实 WeGame、4K 校准、全局快捷键或性能验收。项目无商业代码签名，公开版本边界仍以 v0.1.11 为准。
...
2026-08-13 `v0.1.12` Windows 候选基线（workflow_dispatch 全绿；尚未 tag / Release）：
...
- Windows candidate workflow 已全绿，但实时用户 4K / DPI / 多屏校准、真实 WeGame 交接 / LCU 恢复、Windows 全局快捷键和游戏性能采样仍未完成；title-only ONNX fixture、1024×768 packaged 校准和合成 updater 不等于这些实机闭环。HB-020 / HB-022 / HB-023～HB-027 以及 UI 长期目标状态均不得升级为 `FIXED` / `VERIFIED`。
...
- HB-023：当前 `IN PROGRESS`。workflow 已删除远端 cleanup并通过 Windows retention / pack / metadata / checksums 门禁，本地 cleanup 受精确目录 / symlink / 越界守卫；仍需正式 tag 后验证历史 v0.1.11 Release / assets确实保留，以及真实 installed N→N+1 的旧 / 新 blockmap、`Range` / `206`、传输量、fallback 与安装验证。
...
- 远端保留与差分发布契约（HB-023，candidate）：workflow 中旧的远端 Release cleanup 已删除，并在 test / pack 前运行静态 verifier 拒绝已知 Release / tag 删除命令；v0.1.11 及以后全部正式 Releases、EXE / ZIP / blockmap / metadata / checksum assets 和 tags 仍必须永久保留。所有受支持源版本的 versioned blockmap 应随其 Release 保留，必要时再同步到独立、不可变的官方稳定通道；新版本发布还须回读确认旧 / 新 blockmap 后才原子更新 `latest.yml`。retention verifier 与 Windows pack / metadata / checksums 已通过，但 verifier 不是完整语义审计，且尚未通过正式 tag 发布验证远端 v0.1.11 保留。
