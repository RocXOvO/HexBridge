...
> 最后更新：2026-08-13
> 当前基线：公开最新且唯一保留的正式 Release 为 `v0.1.9`，tag / 产品源码固定指向 `8a6e6d20791f0596274b79704d229642b99a7a12`，公开 `update-channel/latest.yml` 已独立核验指向 `0.1.9` / 199,026,359 bytes。正式 Windows tag workflow 全门禁成功；旧 GitHub Release 记录及资产已按策略清理，但 `v0.1.0` 至 `v0.1.9` Git tags 与源码历史全部保留。HB-020 与 HB-022 均保持 `IN PROGRESS`。用户新日志进一步暴露 LCU authority 绑定高风险，相关修复已进入本地 `v0.1.10` 候选；独立审查无 P0 / P1，本地 131 passed + 1 Windows-only skipped 及 source / 静态门禁通过，但尚未 commit / push / Windows workflow / tag / Release。v0.1.9 的 CI 与 v0.1.10 的本地回放都不等于报告用户同机真实 WeGame 选人与交接复验，不能标 `FIXED` / `VERIFIED`。项目无商业代码签名。
...
- `v0.1.10` 当前验证：新增 production `LcuClient → HexBridgeRuntime` 回放，覆盖 authority alias / rotation、外部 authority 干扰、可信交接推进、同局最后换英雄、active 后同英雄无 identity 的第二局以及终局 / 新局边界。当前本地为 15 test files / 131 passed + 1 Windows-only skipped，typecheck、lint、`git diff --check`、source bridge / UI smokes 全部通过；独立审查无 P0 / P1，批准进入 Windows 候选验证。版本已升为 `0.1.10`，但尚未 commit / push，Windows workflow 尚未运行，也未 tag / Release；公开最新仍为 v0.1.9。production 回放仍是受控数据，不运行真实 WeGame，不能升级 HB-022 状态。
...
- HB-022：当前 `IN PROGRESS`。v0.1.9 发布后用户日志仍显示两个 candidate 中选中 log source 且 transport-connected 后 raw / normalized phase 长期 `None`，authority 绑定风险未完成实机闭环。`v0.1.10` 本地候选新增 authority alias / 世代隔离、统一 retained priority、transport / match lease 分离、外部 authority 防覆盖、可信交接推进、最后换英雄优先采纳及第二局 / 终局 generation 边界；15 files / 131 passed + 1 skipped、source / 静态门禁和独立审查通过。Windows、commit / push / tag / Release 与报告用户同机复验均未完成，不能标 FIXED。
...
- 当前 Git / 版本：公开最新且唯一保留的 Release 仍为 `v0.1.9`，tag / 产品源码为 `8a6e6d20791f0596274b79704d229642b99a7a12`，public channel 为 `0.1.9`。Release 公开、非 draft、非 prerelease；旧 Release / assets 已清理，但远端 `v0.1.0` 至 `v0.1.9` Git tags 和源码历史全部原位保留，`v0.1.4` 从未有 Release。当前本地版本已升 `0.1.10`，尚未 commit / push / Windows workflow / tag / Release，不得描述为公开版本或预写结果；不得改写既有产品 tag。
