...
- 当前候选为 `v0.1.59`，公开正式版为 [v0.1.58](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.58)；本地全量 `50` files / `628` passed + `1` skipped，typecheck、lint、diff-check 与渲染/增量 OCR 定向回归通过。候选移除整组三卡外层过渡，短暂隐藏/恢复保留可靠槽位签名，只让变化卡片动画；真实 WeGame 单卡刷新、帧时间仍 `UNVERIFIED`。v0.1.59 已 commit / push，尚未 Windows workflow / tag / Release。
- 当前公开正式版为 [v0.1.58](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.58)，Release ID（API node）`371198640`，publishedAt `2026-08-16T01:08:34Z`；annotated tag object `c885d386ecda256720b9af5daf15c4cb4b192e15` 解引用到 commit `e4b9deff7931d10a778b57276b02b831176d5040`。正式 workflow `31918661001` / job `95095618306` 首次稳定通道传播超时后幂等重跑成功；Windows 全链、五资产、v2/root 双通道、public packaged 与五版滚动保留通过。真实 WeGame 单卡刷新、帧时间仍待用户复测，HB-092 保持 `FIXED / UNVERIFIED`。
...
- v0.1.59 候选移除三卡推荐区整组三卡外层过渡；短暂隐藏/恢复保留可靠槽位签名，只有变化卡片重播动画，三卡真正清空才重置签名。真实 Windows / WeGame 视觉仍未验。
...
- GitHub 当前只保留 v0.1.53、v0.1.55、v0.1.56、v0.1.57、v0.1.58 五个正式 Releases（v0.1.54 仅 tag）；v0.1.0～v0.1.58 tags 全部保留。本地 `release/` 为空；旧 Release / assets 按滚动策略删除但 tag / source 保留。
