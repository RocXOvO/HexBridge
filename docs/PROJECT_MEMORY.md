...
- 当前公开正式版为 [v0.1.60](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.60)；本版只在显式可选卡诊断采样中增加一次性 `allgamedata`，固定本机 `127.0.0.1:2999`、2 MiB / 超时 / Main-only 脱敏；不接入后台轮询、不把等级或事件字段当作选卡状态。用户已提供的三份真实摘要尚未发现稳定选卡字段；真实 Windows 三时间点采样仍待用户提供，HB-079 保持 `IN PROGRESS / UNVERIFIED`。
- v0.1.61 当前为本地候选：诊断页新增 Windows 个人研究模式，按三个时间点显式把完整 `allgamedata` 写入用户数据目录的私有实验子目录；Renderer / IPC 返回仅有成功状态、文件名和字节数，日志、网络、RuntimeState 与 Release 不含原文，并提供清除按钮。未 commit / push / tag / Release；不得预写 Windows 或真实字段验证。
