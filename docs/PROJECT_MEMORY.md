...
> 最后更新：2026-08-13
> 当前基线：公开最新正式 Release 为 [v0.1.13](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.13)，公开、Latest、non-draft、non-prerelease。annotated tag object `6d38e740…` 指向产品提交 `9ef1a11e…`；正式 tag workflow run `31690145526` / job `94415337106` success，约 5m25s，Windows 23 test files / 200 tests、OCR 真实 4K、lint、typecheck、retention、pack / metadata、packaged UI / bridge、synthetic + public updater、checksums、Release 与 channel 全通过。public channel 核验为 `0.1.13 / 199,181,321 bytes`。v0.1.11、v0.1.12 与 v0.1.13 的正式 Release 及各自五项资产均保留，全部 Git tags 也保留；真实差分网络传输仍未验证。正式产物未做商业代码签名，SmartScreen / 未知发布者边界不变。用户明确确认 v0.1.13 同机中“选人结束→独立游戏客户端启动→英雄丢失”已不再发生，脱敏时间线也证明 3270 最终英雄以同一 generation 从 selecting 经 launching 连续进入 InProgress；HB-020 / HB-022 的该具体交接子项通过，`normalize` / handoff reducer 已冻结且本轮候选未改动。完整终局语义、第二局与 OCR 结果链仍缺证据，两项总体继续 `IN PROGRESS`。当前 v0.1.14 候选提交 `ba5b8d1` 已 push main，`package.json` / lockfile / README 及 smoke / demo 版本已同步；候选已移除 augment BrowserWindow、`#augment` route 与 `AugmentOverlay.vue`，OCR 结果只同步到主窗口且不因同步而 show / focus；按钮、快捷键、托盘统一走 Main 手动 OCR 路径，并收敛稳定诊断与脱敏日志。data.dtodo 英雄专属 `pickRate` 已按窄白名单进入候选，rank 排序不变，详情缓存升为 v2；Tier 背景条恢复肉眼可见原始 `Tn`，移除“强度顶尖”。用户随后在 Windows 右下角托盘右键“退出”时 100% 触发主进程 `TypeError: Object has been destroyed`，已确认是 BrowserWindow 销毁与 `activityChanged → Runtime.sync → WindowManager.sync` 的生命周期竞态，登记为 HB-032。当前 `ba5b8d1` 之上的 dirty 草案已让托盘退出先 `prepareToQuit`，WindowManager 在 quitting 中切断 activity、sync fail-closed、过滤 destroyed window 并在 `closed` 时移除 map 引用，Runtime.stop 同样先进入退出态；source bridge smoke 用真实 BrowserWindow 覆盖 prepare→destroy→late sync。后续审查 P1 将退出建模为两阶段：WindowManager 以 `preparedInstallToken` 表示可撤销的 update install prepare，以 `quitCommitted` 表示不可撤销的真正退出；`prepareForUpdateInstall` 返回 token，`prepareToQuit` commit 并清 token，cancel 只在 token 匹配且尚未 commit 时生效。UpdateManager 持有 token，在同步 throw、安装期 error 或 `quitAndInstall` 返回后检测到 error 时取消；校准继续使用 lifecycle epoch，在入口和各 await 后守卫，prepare 时递增 epoch、销毁 live 校准，退出期拒绝 createWindow。最终复审无 P0 / P1并批准进入 Windows；最新完整链为 25 test files passed、219 tests passed + 1 Windows skip（总计 220），并覆盖旧 token 不能撤销新 token。source Electron UI、真实 4K OCR、Release retention / icon、typecheck、lint、source Electron bridge 与 diff-check 全通过。Windows retry 尚未运行，HB-032 必须保持 `IN PROGRESS`。workflow_dispatch run `31695550346` 已取消，v0.1.14 尚未 tag 或 Release；真实游戏前台快捷键和报告用户同机 OCR 呈现也未验证。HB-023～HB-032 不得因本地门禁或局部实机结果写成整体 `FIXED` / `VERIFIED`；Node 20 deprecated annotation 为非阻断维护项。
...
- 详情缓存 schema 契约：英雄详情缓存必须有独立本地 schema 版本，不得只依赖上游 `dataVersion`。加入英雄专属 `pickRate` 后必须提升 schema；旧缓存即使 `dataVersion` 相同，也要判定为旧 schema 并重新获取 / 迁移，不能永久命中缺少该字段的旧详情。缓存回退仍须标 stale，schema / 数值校验失败不得伪装成 0%。
...
- 通道 / 发布安全实现：通道发布 / 读取遵守单调版本。发布前 preflight 拒绝低于 public channel 的候选，并在任何 Release / channel 写入前分页枚举公开 stable Releases，发现更高版本立即 fail closed；远端 Release 若已是同版本，则要求五项资产及 metadata 全部一致后才 no-op；候选版本的 Release 已存在但不满足同版本一致性时拒绝覆盖。softprops 配置 `overwrite_files:false`；GitHub Actions concurrency 使用 `queue` 且 `max` 有界。`update-channel` 当前公开指向 `v0.1.13`。旧 workflow 曾删除低版 stable Release / assets；该策略已移除，v0.1.13 发布后 v0.1.11 / v0.1.12 仍完整保留。以后所有正式 GitHub Releases / assets / tags 永久保留，任何 workflow 都不得删除远端历史 Release。
- 当前验证：`v0.1.13` 已完成正式 Windows tag workflow、public channel / packaged public check 与五项 Release assets，且 v0.1.11 / v0.1.12 Release 及各自五资产仍在；已删除的 v0.1.0～v0.1.10 Release / assets 不可恢复且不得伪称存在。installed 旧版本对真实 GitHub 的 check / download / `quitAndInstall` / UAC / 替换仍未完成，因此 HB-021 整体不得写为 `VERIFIED`。旧版本用户可从当前 Release 页手动覆盖安装 v0.1.13。
...
- 当前发布实现：release workflow 中旧的 `gh release delete` 远端 cleanup 已移除，CI 增加静态 retention verifier，拒绝 workflow 出现远端 Release / tag 删除命令。`pack:win` 在构建前运行本地 `release/` cleanup：目标固定为仓库根下精确 `release` 目录，拒绝该目录为符号链接或目标逃逸，随后清空其中既有条目，使本次构建结束后只留下当前产物；测试断言目录外文件不受影响。当前采用“精确目录预清空”而不是按 semver 逐文件保留窗口；v0.1.13 正式 Windows workflow 已通过且 v0.1.11 / v0.1.12 远端资产保留，但仍没有真实旧 / 新 blockmap 网络请求或差分下载证明。
...
- 必须验证的验收标准：在真实 Windows installed packaged N→N+1 上分别清空 updater cache 与保留已有 cache 执行 public 更新；两种路径都必须观察到旧 / 新 blockmap 请求和 HTTP `Range` / `206`，实际 installer 网络传输量显著小于完整 EXE，最终 SHA-512 校验、显式安装、重启后的版本替换均通过，并断言未触发 full-download fallback。另行破坏 / 移除 blockmap及模拟服务器不支持 Range，验证 UI 准确切换到完整包、完整下载与安装仍安全成功且不误报差分；再覆盖保留窗口内跨多版本与窗口外版本的明确策略。只有保留通道、客户端地址约束、UI 和上述真实网络 / 安装门禁全部完成，才可升级状态。
...
- 状态：`IN PROGRESS`（用户 Windows 同机 100% 复现，生命周期竞态已确认；尚无修复或验证结果）
...
- 正式 Release [v0.1.12](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.12) 发布时为公开 Latest，非 draft / prerelease；public channel verifier 当时核验 `version=0.1.12`、EXE size 199,180,739 bytes，metadata 与正式资产一致。当前 Latest 已由 v0.1.13 接替，但 v0.1.12 Release / 资产继续保留。
- 正式五项资产：
...
- **更新与差分（HB-019 / HB-021 / HB-023）**：v0.1.11、v0.1.12 与 v0.1.13 远端 Release / 五资产并存只证明 retention 连续生效；仍未验证 installed N→N+1 的真实 GitHub check / download / `quitAndInstall`、UAC / SmartScreen、旧 / 新 blockmap、Range / 206、实际差分字节和安全完整包 fallback。
...
- **视觉验收**：独立更新页、英雄榜减法 / 搜索 / Tier 色带、英雄原画、空态轨道动效、新配色和 icon 已进入 v0.1.12，自动视觉策略在 v0.1.13 继续收敛；仍无覆盖 Windows EXE / 任务栏 / 托盘 / 安装器四处图标、长中文、100%～150% DPI、4K、reduced-motion 与后台重绘的用户同机人工验收。发布与 source / packaged smoke 不能替代视觉验收。
...
- HB-021：当前 `IN PROGRESS`。`v0.1.13` 延续 raw stable channel、GitHub fallback、官方资产 allowlist、错误分类、provider 结果绑定、单调 / 并发保护，public channel 和 packaged public smoke 已通过；仍缺 installed 旧版本真实 check / download / `quitAndInstall` / UAC / 实际替换链。旧版本用户可从当前 Release 页手动覆盖安装 v0.1.13。
...
- HB-023：当前 `IN PROGRESS`。v0.1.13 正式发布后 v0.1.11 / v0.1.12 Release 与各自五资产仍完整保留，证明远端 retention 继续生效且旧 blockmap 可获取；本地 cleanup 受精确目录 / symlink / 越界守卫。仍需真实 installed v0.1.11 或 v0.1.12→v0.1.13 的旧 / 新 blockmap 请求、`Range` / `206`、实际传输量、无 full-download fallback、SHA-512 与安装验证，不能据 retention 单独写 `FIXED` / `VERIFIED`。
...
- HB-026：当前 `IN PROGRESS`。v0.1.12 默认 `autoOcr=false`；v0.1.13 revision 2 继续保留该设置并把旧视觉 override 迁移为 auto，普通 UI / IPC 已不再暴露四档手动选择。显式开启自动 OCR 后每 2 秒先 960px 低分辨率标题 ROI 门控、命中才取 1920px OCR 帧，手动只取一次 1920px。最终审查与正式 Windows 门禁通过，仍缺持久化重启、hidden pause / 退避与真实 4K FPS / frametime / CPU / GPU 对照，未采样前不得确认根因或修复；自动视觉策略剩余验收另由 HB-028 跟踪。
...
- HB-029～HB-031 当前均为 `IN PROGRESS`。v0.1.14 候选已删除 augment BrowserWindow / `#augment` route / `AugmentOverlay.vue`，OCR 结果只进入实时助手且状态同步不 show / focus；已按数值与来源 / 区域 allowlist 清洗英雄专属 `pickRate`，rank 排序不变，详情缓存升为 v2且 legacy 只回退 stale rank / tier；Tier 背景条显示肉眼可见原始 `Tn` 并移除“强度顶尖”。本轮 clean `npm ci` / audit 0、此前完整链 24 files / 210 passed + 1 Windows skip、真实 4K ONNX、typecheck / lint / build / preload / source Electron bridge / UI、Release retention / icon 与 diff-check 均通过，首轮审查 3 个 P1 已修且最终审查无 P0 / P1；新增 `renderer-contracts.test.ts` 随候选提交但未计入该既有完整计数。Windows run 因新发现 HB-032 已取消，游戏前台快捷键和报告用户同机 OCR / 数据 / 视觉仍未验证，三项不得写 `FIXED` / `VERIFIED`。
- HB-032 当前为 `IN PROGRESS`。v0.1.14 候选提交 `ba5b8d1` 已 push 后，用户 Windows 同机从托盘退出 100% 触发 `Object has been destroyed`；受控调用链确认 BrowserWindow 销毁与 activityChanged / Runtime sync / WindowManager sync 发生生命周期竞态。`ba5b8d1` 之上的 dirty 草案已加入 `prepareToQuit`、quitting activity 切断、sync fail-closed、destroyed 引用清理和真实 BrowserWindow late-sync smoke；后续审查 P1 又以 `preparedInstallToken / quitCommitted` 区分可撤安装准备和不可撤退出，并覆盖 updater error rollback、校准 lifecycle epoch / await 守卫。最终复审无 P0 / P1，最新完整 25 files / 219 passed + 1 skip 及 source UI、真实 4K OCR、retention / icon、typecheck / lint / source bridge / diff 门禁全通过。workflow_dispatch run `31695550346` 已取消，Windows retry 与用户复测仍未完成，不得写 `FIXED` / `VERIFIED`。
...
- 当前 Git / 版本：公开 Latest 为 `v0.1.13`，annotated tag object `6d38e740…` 指向产品提交 `9ef1a11e…`，public channel 为 `0.1.13 / 199,181,321 bytes`；Release 公开、non-draft、non-prerelease。正式 tag run `31690145526` / job `94415337106` 已通过完整门禁。v0.1.11、v0.1.12 与 v0.1.13 三个正式 Release 及各自五项资产同时保留；`v0.1.0`～`v0.1.10` 中已发布版本的 Release / assets 因历史错误 cleanup 已删除且不可恢复，但全部 Git tags 与源码历史仍原位保留。后续必须继续累积保留 v0.1.11 及以后全部正式 Releases / assets / tags；不得移动或改写任何已发布产品 tag。
...
- 远端保留与差分发布契约（HB-023）：workflow 中旧的远端 Release cleanup 已删除，并在 test / pack 前运行静态 verifier 拒绝已知 Release / tag 删除命令；v0.1.11 及以后全部正式 Releases、EXE / ZIP / blockmap / metadata / checksum assets 和 tags 必须永久保留。v0.1.13 正式发布后 v0.1.11 / v0.1.12 Release 与各自五资产仍在，已验证本轮没有破坏远端保留；静态 verifier 仍不是完整语义审计。所有受支持源版本的 versioned blockmap 应随 Release 保留；真实差分还必须验证客户端请求旧 / 新 blockmap、HTTP Range / 206、传输字节、fallback 和安装，SHA-512 / blockmap存在本身不能替代该证据。
...
- `v0.1.0` 曾含 HB-012 preload 阻断缺陷，其 Release 已删除，不能作为功能可用基线。当前公开推荐版本为 `v0.1.13`；但 tagged packaged smoke 通过仍不代表真实 WeGame / LCU / 对局 OCR 实机验收。
- `v0.1.13` 是当前公开推荐版本；v0.1.11、v0.1.12 与 v0.1.13 Release / 五资产同时存在，证明新远端保留策略连续生效。HB-020 与 HB-022 仍处于 `IN PROGRESS`；v0.1.13 用户同机已通过交接连续性子项，但完整终局与第二局仍未闭环。HB-014 仅在首帧黑屏 / packaged Windows 受控截图与 Esc 恢复窄范围 `VERIFIED`；HB-013、HB-015～HB-018 及 HB-014 的实机剩余范围仍为 `FIXED / UNVERIFIED`。HB-019 仅 local-feed check / download / SHA-512 / cache 窄范围 `VERIFIED`，真实 installed 客户端更新 / 安装链仍未验证。
- HB-021 当前为 `IN PROGRESS`：`v0.1.13` 已正式发布，public channel 与 packaged public check 已指向 `0.1.13`；但旧 installed 客户端的真实 GitHub check / download / `quitAndInstall` / UAC / 版本替换仍未完成。不得把 workflow、synthetic updater 或服务端资产存在等同于真实客户端升级已验证；旧版本用户可手动覆盖安装 v0.1.13。
- HB-023 当前为 `IN PROGRESS`：v0.1.13 workflow 延续禁止远端 cleanup 的 retention 规则，retention verifier 与受限本地 pre-pack cleanup 已进入正式链；远端 v0.1.11 / v0.1.12 / v0.1.13 Release 及各自五资产同时存在，证明本轮未删除旧版，历史已删资产仍不可恢复。这不等于未来所有版本的永久保留已经被语义证明，更不等于差分下载已发生；在真实 public N→N+1 证明旧 / 新 blockmap、`Range` / `206`、差分字节、fallback 与安装前，不得宣称差分更新 `FIXED` / `VERIFIED`。
...
- GitHub Actions 仍有 Node 20 deprecated annotation。当前 workflow 中第三方 actions 固定到完整 commit SHA，runner 已临时强制它们使用 Node 24，且 `v0.1.13` 正式 run `31690145526` 成功，因此不是当前发布阻断项。
...
- 2026-08-13 | HB-020 / 实机交接 runbook | IN PROGRESS（等待用户执行） | 新增 `docs/WEGAME_HANDOFF_RUNBOOK.md`，把真实同机状态链、不变量、失败判据、终局 / 第二局、脱敏字段与 FIXED / VERIFIED 升级门槛写为仓库契约；README 提供入口 | `git diff --check` 通过；内容与 Runtime 受控诊断字段及项目记忆一致 | 文档不能替代用户实际运行，目标仍未完成 | 待报告问题的同一机器手动覆盖安装当前 v0.1.13 后执行
...
- 2026-08-13 | HB-032 / 两阶段退出最终本地候选 | IN PROGRESS（最终复审/本地全绿；不得 FIXED / VERIFIED） | WindowManager以preparedInstallToken表示可撤安装准备、quitCommitted表示不可撤退出；prepareForUpdateInstall返回token，prepareToQuit commit并清token，cancel仅匹配token且未commit有效。UpdateManager持token并在同步throw/安装期error/返回后检测error时取消；Runtime接prepare/cancel；校准lifecycle epoch继续守卫异步与窗口存活 | 最终复审无P0/P1并批准Windows；25 files passed、219 tests pass+1 Windows skip，覆盖旧token不能撤销新token；source UI、真实4K OCR、retention/icon、typecheck/lint/source bridge/diff全过 | Windows retry和报告用户同机托盘退出复测未完成；本地/复审结果不得外推为修复完成 | 草案仍在`ba5b8d1`之上dirty；run `31695550346`已取消，不tag/Release，公开Latest仍v0.1.13
