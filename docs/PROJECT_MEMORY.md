...
> 当前 Windows 候选：v0.1.16 产品 commit `e37765620db6d36c070ee602c3ddee06ce5049ca` 已 push main；workflow_dispatch run `31718519456` / job `94509290728` success，约 5m32s。clean `npm ci`、Electron hydrate、audit、public legacy check、OCR synthetic + 真实 4K fixture、Windows 29 test files / 255 passed（Windows-only 项已实际运行，无 skip）、lint / typecheck、retention、pack / updater metadata、packaged UI / bridge（含生产 `NativeImage` 裁切）、differential updater smoke、checksums 与 artifact 全通过；真实 4K fixture 在该 Windows run 为 270ms，候选 EXE metadata size 为 199,235,065 bytes。差分 smoke 从 previous 0.1.15 合成 available 0.1.17，`differentialDownload=true`，old / new blockmap 各1次、Range 请求10次，传输 1,213,091 / 199,235,065 bytes，`isolatedCache=true`。tag-only Release / channel 步骤按预期 skip，v0.1.16 尚未创建 tag / Release，公开 Latest 仍为 v0.1.15。用户脱敏日志已覆盖连续两局的英雄保留、终局清理与第二局换代，并在 23:12 记录热键手动 OCR 返回 `BUSY`；这些与 Windows runner 都不替代真实 Windows 游戏 frametime / CPU / GPU 和报告用户同机 OCR 验收。HB-024～026 不得标 `VERIFIED`，HB-026 继续 `IN PROGRESS`。
...
- v0.1.16 候选验证：本地 29 test files / 254 passed + 1 skipped，typecheck、lint、`git diff --check`、真实 4K fixture（最近一次 134ms）、source bridge / UI smoke 和 version v0.1.16 gate 全通过；独立最终审查 P0 / P1 为 0。随后产品 commit `e37765620db6d36c070ee602c3ddee06ce5049ca` 已 push main，Windows workflow_dispatch run `31718519456` / job `94509290728` success，约 5m32s：Windows 实际运行 29 test files / 255 passed（无 skip），真实 4K fixture 270ms，packaged bridge 覆盖生产 `NativeImage` 裁切，候选 EXE metadata size 199,235,065 bytes；其余 clean dependencies / hydrate / audit / public legacy / OCR / lint / typecheck / retention / pack / metadata / packaged UI / differential updater / checksums / artifact 门禁均通过。这不是真实 Windows 游戏性能验收；尚未 tag / Release，报告用户同机性能数据仍缺，因此 HB-026 保持 `IN PROGRESS`。
...
- 下一版验收边界：必须由真实 Windows installed v0.1.15 执行 v0.1.15→v0.1.16（或下一正式版本）更新，分别覆盖没有可用旧 installer cache 与已有 cache。记录脱敏状态时间线和实际网络累计字节，确认 UI 显示的是 metadata full size、预计差分量还是已下载量；同时核对旧 / 新 blockmap、Range / 206、是否发出完整 installer 请求、SHA-512、显式安装确认、UAC、替换 / 重启后版本。无 cache 时若设计上必须回退完整包，UI 要明确说明；若产品承诺从稳定通道恢复旧 installer，则必须验证恢复来源与摘要。另覆盖 blockmap 缺失 / 损坏、Range 不支持和差分应用失败，确认中文提示与安全 fallback。不得记录带查询参数 URL、token、用户路径或安装目录。
...
- 本次为无 tag 候选 workflow，tag-only Release / channel 步骤按预期 skip；尚未创建 v0.1.16 tag / Release，公开 Latest 仍为 v0.1.15。Windows runner 不是真实 Windows 游戏 frametime / CPU / GPU 验收，HB-026 继续 `IN PROGRESS`。
...
- **v0.1.16 Windows 候选**：commit `e37765620db6d36c070ee602c3ddee06ce5049ca` 已 push main；run `31718519456` / job `94509290728` success，约 5m32s。Windows 29 files / 255 passed（无 skip）、真实 4K fixture 270ms、packaged UI / bridge 含生产 `NativeImage` 裁切、EXE metadata size 199,235,065 bytes，以及 previous 0.1.15 → synthetic 0.1.17 的差分烟测（old / new blockmap 各1、Range 10、传输 1,213,091 / 199,235,065 bytes、isolated cache）通过。该证据不测量真实游戏 FPS / frametime / CPU / GPU，不替代报告用户同机 OCR / 性能复测；HB-026 保持 `IN PROGRESS`。tag-only 步骤 skip，v0.1.16 尚未发布，Latest 仍为 v0.1.15。
...
- HB-033～HB-038 当前均为 `IN PROGRESS`。v0.1.15 已公开发布，正式 run 在人工 seed 旧 installer cache 的条件下验证 Range / blockmap / 零完整 installer / 低于 25% 的差分门禁，并通过 public channel / packaged check；但用户 v0.1.14→v0.1.15 仍看到 / 可能下载约 200 MB，尚无网络字节证据区分 metadata 展示与 full fallback。必须由 installed v0.1.15 对下一正式版本复验；Windows 图标、合成器隐藏截图恢复，以及报告用户同机多轮三卡 / 中文依据 / toast 也仍未验。公开 Latest 为 v0.1.15。
...
- v0.1.16 候选状态：产品 commit `e37765620db6d36c070ee602c3ddee06ce5049ca` 已 push main；workflow_dispatch run `31718519456` / job `94509290728` success，约 5m32s，且无 tag 情形下的 Release / channel 步骤按预期 skip。当前尚未创建 v0.1.16 tag / Release，所以公开 Latest 和 public channel 仍是 v0.1.15；不得将 Windows 候选成功写成已发布。
...
- HB-023 当前为 `IN PROGRESS`：v0.1.15 workflow 延续禁止远端 cleanup 的 retention 规则，retention verifier 与受限本地 pre-pack cleanup 已进入正式链；远端 v0.1.11～v0.1.14 Releases 仍在，至少 v0.1.12～v0.1.15 五资产已记录，证明本轮未删除旧版，历史已删资产仍不可恢复。正式 Windows / public 门禁在人工 seed 旧 installer cache 时得到 old / new blockmap、Range 请求、零完整 installer 与显著较低传输量的窄证据；这不等于报告用户 installed 安装、fallback、UAC、进程替换与重启已发生。用户 v0.1.14→v0.1.15 仍看到 / 可能下载约 200 MB，必须等 installed v0.1.15→下一正式版的同机请求 / 字节证据后再判断，当前不得宣称差分更新整体 `FIXED` / `VERIFIED`。
...
- GitHub Actions 仍有 Node 20 deprecated annotation。当前 workflow 中第三方 actions 固定到完整 commit SHA，runner 已临时强制它们使用 Node 24；`v0.1.15` 正式 run `31708642394` 与 v0.1.16 候选 run `31718519456` 均成功，因此该 annotation 仍为非阻断维护项。
