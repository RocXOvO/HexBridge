...
> 最后更新：2026-08-13
> 当前基线：公开最新且唯一保留的正式 Release 为 `v0.1.9`，tag / 产品源码固定指向 `8a6e6d20791f0596274b79704d229642b99a7a12`，公开 `update-channel/latest.yml` 已独立核验指向 `0.1.9` / 199,026,359 bytes。正式 Windows tag workflow 全门禁成功；旧 GitHub Release 记录及资产已按策略清理，但 `v0.1.0` 至 `v0.1.9` Git tags 与源码历史全部保留。HB-020 与 HB-022 均保持 `IN PROGRESS`。`v0.1.10` candidate commit `cb2098c79842f61447ab933766b42ff45c1604c5` 已 push main，Windows workflow_dispatch 以 15 files / 132 tests 和完整候选门禁成功；但尚未 tag / Release，公开 Latest / channel 仍为 v0.1.9。Windows runner 与受控回放不等于报告用户同机真实 WeGame 选人与交接复验，不能标 `FIXED` / `VERIFIED`。项目无商业代码签名。
...
- 通道 / 发布安全实现：通道发布 / 读取遵守单调版本。发布前 preflight 拒绝低于 public channel 的候选，并在任何 Release / channel 写入前分页枚举公开 stable Releases，发现更高版本立即 fail closed；远端 Release 若已是同版本，则要求五项资产及 metadata 全部一致后才 no-op；候选版本的 Release 已存在但不满足同版本一致性时拒绝覆盖。softprops 配置 `overwrite_files:false`；GitHub Actions concurrency 使用 `queue` 且 `max` 有界。`update-channel` 当前公开指向 `v0.1.9`。正式发布完成 public packaged 检查后只删除严格低版 stable Release / assets；cleanup 再次拒绝更高 stable，draft / prerelease 不删，所有 tags / 源码历史永久保留。
- 当前验证：`v0.1.9` 已完成 Windows 预发布与 tag workflows，public channel / packaged public check 和五项 Release assets 均完成；目前只保留该 Latest Release。installed 旧版本对真实 GitHub 的 check / download / `quitAndInstall` / UAC / 替换未完成，因此 HB-021 整体仍不得写为 `VERIFIED`。旧版本用户此次建议从当前 Release 页手动下载安装 v0.1.9。
...
- 状态：`IN PROGRESS`（`v0.1.9` 发布后用户日志暴露 authority 绑定高风险；`v0.1.10` 本地候选待 Windows / 同机实机验证）
...
- `v0.1.10` 当前验证：新增 production `LcuClient → HexBridgeRuntime` 回放，覆盖 authority alias / rotation、外部 authority 干扰、可信交接推进、同局最后换英雄、active 后同英雄无 identity 的第二局以及终局 / 新局边界。独立审查无 P0 / P1。candidate commit `cb2098c79842f61447ab933766b42ff45c1604c5` 已 push main；Windows workflow_dispatch run [31665154616](https://github.com/RocXOvO/HexBridge/actions/runs/31665154616) / job [94338020977](https://github.com/RocXOvO/HexBridge/actions/runs/31665154616/job/94338020977) 成功，用时约 5m2s。clean `npm ci`、Electron hydrate、audit 0、public `0.1.9` channel verify、OCR、Windows 15 test files / 132 tests、lint、typecheck、pack（EXE 199,028,315 bytes）、metadata、packaged UI（1024×768 校准）、bridge（含 Windows LCU discovery 策略门禁）、checksums 和 artifact 全部通过；synthetic updater `0.1.11` 得到 `downloaded=true`、metadata / installer 各 1 次、`isolatedCache=true`。tag-only Release、channel 写入与 cleanup 按预期 skip。当前尚未 tag / Release，公开最新仍为 v0.1.9；Windows runner / production 回放不运行真实 WeGame，不能升级 HB-022 状态。
...
`v0.1.7` / `v0.1.8` 历史正式发布与当前 `v0.1.9`：
...
- `v0.1.9` 当前为公开、非 draft / prerelease 且唯一保留的正式 Release，tag / 产品提交 `8a6e6d20791f0596274b79704d229642b99a7a12`。正式 tag run [31663071062](https://github.com/RocXOvO/HexBridge/actions/runs/31663071062) / job [94331796412](https://github.com/RocXOvO/HexBridge/actions/runs/31663071062/job/94331796412) 于 2026-08-13T03:09:58Z～03:15:40Z 成功，约 5m42s；113 passed + 1 Windows skip、packaged UI / bridge、synthetic updater、public channel、Release 与旧 Release cleanup 全部成功。public stable channel 已独立核验为 `0.1.9`，EXE size `199026359`。
...
- HB-021：当前 `IN PROGRESS`。`v0.1.9` 延续 raw stable channel、GitHub fallback、官方资产 allowlist、错误分类、provider 结果绑定、单调 / 并发保护，public channel 和 packaged public smoke 已通过；仍缺 installed 旧版本真实 check / download / `quitAndInstall` / UAC / 实际替换链。旧版本用户此次建议从当前 Release 页手动下载安装 v0.1.9。
- HB-022：当前 `IN PROGRESS`。v0.1.9 发布后用户日志仍显示两个 candidate 中选中 log source 且 transport-connected 后 raw / normalized phase 长期 `None`，authority 绑定风险未完成实机闭环。`v0.1.10` 候选新增 authority alias / 世代隔离、统一 retained priority、transport / match lease 分离、外部 authority 防覆盖、可信交接推进、最后换英雄优先采纳及第二局 / 终局 generation 边界；独立审查无 P0 / P1，Windows workflow_dispatch 的 15 files / 132 tests、packaged UI / bridge / updater 等候选门禁通过。真实 WeGame 报告用户同机复验与 tag / Release 均未完成，不能标 FIXED。
...
- 当前 Git / 版本：公开最新且唯一保留的 Release 仍为 `v0.1.9`，tag / 产品源码为 `8a6e6d20791f0596274b79704d229642b99a7a12`，public channel 为 `0.1.9`。Release 公开、非 draft、非 prerelease；旧 Release / assets 已清理，但远端 `v0.1.0` 至 `v0.1.9` Git tags 和源码历史全部原位保留，`v0.1.4` 从未有 Release。`v0.1.10` candidate commit `cb2098c79842f61447ab933766b42ff45c1604c5` 已 push main 并通过 Windows workflow_dispatch；尚未 tag / Release，不得描述为公开版本或预写正式结果，既有产品 tag 不得改写。
...
- `v0.1.0` 曾含 HB-012 preload 阻断缺陷，其 Release 已删除，不能作为功能可用基线。当前公开推荐版本为 `v0.1.9`；但 tagged packaged smoke 通过仍不代表真实 WeGame / LCU / 对局 OCR 实机验收。
- `v0.1.9` 是当前公开推荐且唯一保留的正式 Release，但 HB-020 与 HB-022 仍处于 `IN PROGRESS`，不能把该版本写成已由报告用户同机验证。HB-014 仅在首帧黑屏 / packaged Windows 受控截图与 Esc 恢复窄范围 `VERIFIED`；HB-013、HB-015～HB-018 及 HB-014 的实机剩余范围仍为 `FIXED / UNVERIFIED`。HB-019 仅 local-feed check / download / SHA-512 / cache 窄范围 `VERIFIED`，真实 installed 客户端更新 / 安装链仍未验证。
- HB-021 当前为 `IN PROGRESS`：`v0.1.9` 已正式发布，public channel 与 packaged public check 已指向 `0.1.9`；但旧 installed 客户端的真实 GitHub check / download / `quitAndInstall` / UAC / 版本替换仍未完成。不得把 workflow、synthetic updater 或服务端资产存在等同于真实客户端升级已验证；此次建议旧版本用户手动下载安装 v0.1.9。
...
- GitHub Actions 仍有 Node 20 deprecated annotation。当前 workflow 中第三方 actions 固定到完整 commit SHA，runner 已临时强制它们使用 Node 24，且 `v0.1.9` 正式 run `31663071062` 成功，因此不是当前发布阻断项。
...
- 2026-08-13 | GitHub Release 保留策略（v0.1.8 发布前时点） | 旧 Release / assets 已删除，tags / 源码历史保留 | 手动删除 `v0.1.0/1/2/3/5/6` GitHub Release 记录与资产，当时只剩 `v0.1.7`；`v0.1.4` 本来无 Release。workflow 仅在新 strict-semver stable 已公开、public packaged 检查通过且没有更高 stable 时，删除严格低版本 stable Release；draft / prerelease 不动，所有 Git tags 永不删除 / 移动 | 当时只读 `gh release list` 确认仅 `v0.1.7`；后续 v0.1.8 正式 run 已按规则删除 v0.1.7 Release / assets | 删除 Release assets 不可恢复；历史下载摘要只作为审计记录，不表示资产仍可下载 | 此条记录历史时点；v0.1.9 发布后当前只剩 v0.1.9 Release，策略不变
...
- 2026-08-13 | HB-020 / 实机交接 runbook | IN PROGRESS（等待用户执行） | 新增 `docs/WEGAME_HANDOFF_RUNBOOK.md`，把真实同机状态链、不变量、失败判据、终局 / 第二局、脱敏字段与 FIXED / VERIFIED 升级门槛写为仓库契约；README 提供入口 | `git diff --check` 通过；内容与 Runtime 受控诊断字段及项目记忆一致 | 文档不能替代用户实际运行，目标仍未完成 | 待报告问题的同一机器手动安装当前 v0.1.9 后执行
