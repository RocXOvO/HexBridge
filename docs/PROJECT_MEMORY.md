...
> 最后更新：2026-08-13
> 当前基线：公开最新且唯一保留的正式 Release 为 `v0.1.10`，annotated tag 固定指向产品 / 记忆提交 `345c0d5443760a9dcc6717a96a6068b6101b16d1`。正式 Windows tag workflow 以 132 tests 和完整发布门禁成功，public channel 的 version / path / size / SHA-512 与正式 EXE 一致。`v0.1.9` Release / assets 已按策略删除，但 `v0.1.0` 至 `v0.1.10` Git tags 与源码历史全部保留。用户 v0.1.10 同机日志已确认自定义海克斯大乱斗使用 `queueId=3270`，旧代码只接受 2400。`v0.1.11` candidate commit `46859c9243ede21628646165d0685ddc1288c7d7` 已 push main，并以 Windows 16 files / 139 passed + 1 skipped 和完整候选门禁通过 workflow_dispatch；tag-only 步骤按预期 skip，尚未 tag / Release。HB-020 与 HB-022 均保持 `IN PROGRESS`，Windows runner 不等于真实 WeGame 同机复测，绝不能标 `FIXED` / `VERIFIED`。公开 Latest / channel 仍为 v0.1.10，项目无商业代码签名。
...
- 状态：`IN PROGRESS`（用户 v0.1.10 日志已确定“3270 未被支持”的主根因；`v0.1.11` 本地候选已通过终审 / 全量 / 交叉打包，尚待 Windows / 同机复测）
...
- `v0.1.11` 当前验证：最终只读复审未发现运行时代码 P0 / P1。clean 本地 `npm ci`、audit 0、OCR、16 test files / 139 passed + 1 Windows-only skipped、lint、typecheck、source bridge / UI、`git diff --check` 全部通过；macOS `pack:win`、metadata verifier 与 checksums exit 0，交叉摘要仅作非正式历史证据。candidate commit `46859c9243ede21628646165d0685ddc1288c7d7` 已 push main；Windows workflow_dispatch run [31667866337](https://github.com/RocXOvO/HexBridge/actions/runs/31667866337) / job [94346222231](https://github.com/RocXOvO/HexBridge/actions/runs/31667866337/job/94346222231) 成功，用时约 5m7s。clean `npm ci`、Electron hydrate、audit 0、public channel `0.1.10` verify、OCR、Windows 16 files / 139 passed + 1 Windows skip、lint、typecheck、pack / metadata、packaged UI / bridge、synthetic `v0.1.12` updater、checksums 和 artifact 全部通过；tag-only Release / channel / cleanup 步骤按预期 skip。Windows runner 不运行真实 WeGame；候选尚未 tag / Release，不能升级 HB-020 / HB-022 状态。
...
- HB-020：当前为 `IN PROGRESS`。除既有迟到空 ChampSelect P1 外，用户 v0.1.10 日志已确定 3270 未被目标队列集合接受是本次自定义局上下文未建立 / 进局丢失的直接根因。`v0.1.11` 候选已统一 `{2400,3270}`，收紧 actions hero fallback，增加 event-only WAMP trailing poll，并以 production 回放覆盖 generation 1→active、transport detach / 双 candidate same-match 重绑和第二局 generation 2；终审、clean 全量和 Windows workflow_dispatch 的 139 passed + 1 skipped / packaged 门禁通过。报告用户同机完整一局 / 终局 / 第二局前不得升级状态。
...
- HB-022：当前 `IN PROGRESS`。用户 v0.1.10 同机日志已确定自定义局 queue 3270 被旧 2400-only 判定拒绝，导致有效英雄不进入 match context；authority 绑定仍是加固背景而非本次主根因。`v0.1.11` 候选已统一 `{2400,3270}`，actions 仅取最新有效本地 pick（pick=0 屏蔽旧 pick / intent、ban 忽略、current / myTeam 优先），event-only WAMP 风暴只补一次 trailing poll；3270 / transport 双候选回放、终审、clean 全量和 Windows workflow_dispatch 候选门禁通过。正式匹配 queue ID、报告用户同机复测与 tag / Release 仍缺失，不能标 FIXED / VERIFIED。
...
- 当前 Git / 版本：公开最新且唯一保留的 Release 为 `v0.1.10`，annotated tag / 产品 / 记忆提交为 `345c0d5443760a9dcc6717a96a6068b6101b16d1`，public channel 为 `0.1.10`。Release 公开、非 draft、非 prerelease；`v0.1.9` Release / assets 已清理，但远端 `v0.1.0` 至 `v0.1.10` Git tags 和源码历史全部原位保留。`v0.1.11` candidate commit `46859c9243ede21628646165d0685ddc1288c7d7` 已 push main 并通过 Windows workflow_dispatch；尚未 tag / Release，不得描述为公开版本或预写 tag 结果，既有产品 tag 不得移动或改写。
