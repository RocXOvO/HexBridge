...
> 最后更新：2026-08-13
> 当前基线：公开最新正式 Release 为 [v0.1.13](https://github.com/RocXOvO/HexBridge/releases/tag/v0.1.13)，公开、Latest、non-draft、non-prerelease。annotated tag object `6d38e740…` 指向产品提交 `9ef1a11e…`；正式 tag workflow run `31690145526` / job `94415337106` success，约 5m25s，Windows 23 test files / 200 tests、OCR 真实 4K、lint、typecheck、retention、pack / metadata、packaged UI / bridge、synthetic + public updater、checksums、Release 与 channel 全通过。public channel 核验为 `0.1.13 / 199,181,321 bytes`。v0.1.11、v0.1.12 与 v0.1.13 的正式 Release 及各自五项资产均保留，全部 Git tags 也保留；真实差分网络传输仍未验证。正式产物未做商业代码签名，SmartScreen / 未知发布者边界不变。用户明确确认 v0.1.13 同机中“选人结束→独立游戏客户端启动→英雄丢失”已不再发生，脱敏时间线也证明 3270 最终英雄以同一 generation 从 selecting 经 launching 连续进入 InProgress；HB-020 / HB-022 的该具体交接子项通过，`normalize` / handoff reducer 已冻结且本轮候选未改动。完整终局语义、第二局与 OCR 结果链仍缺证据，两项总体继续 `IN PROGRESS`。当前 v0.1.14 候选提交 `34d14b45156eb762480c7d13af72dca2fd20ed2b` 已 push main；候选已移除 augment BrowserWindow / route / component、收口 Main 手动 OCR 状态，按窄白名单接入英雄专属 `pickRate` v2 缓存，并恢复肉眼可见原始 Tier，HB-029～031 继续 `IN PROGRESS`。HB-032 的托盘退出生命周期竞态已用两阶段 `preparedInstallToken / quitCommitted`、quitting fail-closed、destroyed 引用清理、校准 epoch 和 updater error rollback 修正；最终复审无 P0 / P1。workflow_dispatch run `31697118111` / job `94437330072` success，约 5m8s：Windows clean `npm ci` / audit 0、public 0.1.13、OCR 合成 + 真实 4K、25 test files / 219 passed + 1 Windows skip、lint、typecheck、retention、pack / metadata、packaged UI、packaged bridge（含 `shutdownLifecycle`）、synthetic updater、checksums 与 artifact 全通过，tag-only 步骤按预期 skip。该证据把 HB-032 提升到 `FIXED / UNVERIFIED`，但 packaged bridge 的生命周期烟测不等于报告用户同机从系统托盘右键退出，仍需用户复测才能进一步验证。v0.1.14 尚未 tag 或 Release，公开 Latest 仍为 v0.1.13；真实游戏前台快捷键和报告用户同机 OCR 呈现也未验证。HB-023～HB-031 不得因候选门禁或局部实机结果写成整体 `FIXED` / `VERIFIED`；Node 20 deprecated annotation 为非阻断维护项。
...
- HB-032 当前为 `FIXED / UNVERIFIED`。v0.1.14 候选提交 `34d14b45156eb762480c7d13af72dca2fd20ed2b` 已 push；最终复审无 P0 / P1，最新完整 25 files / 219 passed + 1 skip 及 source UI、真实 4K OCR、retention / icon、typecheck / lint / source bridge / diff 门禁全通过。Windows run `31697118111` / job `94437330072` 又通过 packaged UI / bridge `shutdownLifecycle`、pack / metadata / updater / checksums / artifact；但尚无报告用户同机系统托盘右键退出复测，不能标 `VERIFIED`。
