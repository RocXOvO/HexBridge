...
describe('stable Release notes', () => {
  ...
  it('selects the immediately preceding public stable version', () => {
    expect(previousStableReleaseTag(releases, '0.1.33')).toBe('v0.1.32')
  })
  ...
  it('renders curated changes relative to the previous stable Release', () => {
    ...
    expect(body).toContain('### 相较 v0.1.32 的更新')
    expect(body).toContain('旧版保存的游戏目录仅留在 Main 进程用于 LCU 发现')
    expect(body).toContain('/compare/v0.1.32...v0.1.33')
    expect(body).not.toContain('v0.1.31：')
  })
